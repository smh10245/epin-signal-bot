    ...
    def stream(self,codes_fn,names,on_tick,on_book,state):
        with self.stream_lock:
            if self.stream_running:
                log.warning('통합 WebSocket 중복 실행 차단')
                return
        ...
        try:
            ...
            while True:
                ...
                subscribed={
                    'KRX':{'trade':set(),'order':set()},
                    'NXT':{'trade':set(),'order':set()},
                }
                duplicate_appkey=False
                try:
                    krx_open=in_session(SETTINGS.krx_start,SETTINGS.krx_end)
                    nxt_open=(SETTINGS.enable_nxt and SETTINGS.kis_env!='virtual' and in_session(SETTINGS.nxt_start,SETTINGS.nxt_end))
                    if not (krx_open or nxt_open):
                        state['ws_krx']='waiting_market_session'
                        state['ws_nxt']='waiting_market_session' if SETTINGS.enable_nxt else 'disabled'
                        time.sleep(30)
                        continue
    
                    key=self.approval_key()
                    state['ws_krx']='connecting' if krx_open else 'waiting_market_session'
                    state['ws_nxt']='connecting' if nxt_open else ('waiting_market_session' if SETTINGS.enable_nxt else 'disabled')
                    state['krx_last_error']=''
                    state['nxt_last_error']=''
    
                    def market_open(market):
                        if market=='KRX':
                            return in_session(SETTINGS.krx_start,SETTINGS.krx_end)
                        return SETTINGS.enable_nxt and SETTINGS.kis_env!='virtual' and in_session(SETTINGS.nxt_start,SETTINGS.nxt_end)
    
                    def tr_ids(market):
                        if market=='KRX':
                            return SETTINGS.krx_trade_tr,SETTINGS.krx_order_tr
                        return SETTINGS.nxt_trade_tr,SETTINGS.nxt_order_tr
                    ...
                    def sync_market(ws,market):
                        trade_tr,order_tr=tr_ids(market)
                        trade_codes,order_codes=desired_codes(market)
                        wanted_trade=set(trade_codes)
                        wanted_order=set(order_codes)
                        current=subscribed[market]
    
                        for code in list(current['order']-wanted_order):
                            send_sub(ws,order_tr,code,'2')
                            current['order'].discard(code)
                            time.sleep(SETTINGS.ws_subscribe_delay)
                        for code in list(current['trade']-wanted_trade):
                            send_sub(ws,trade_tr,code,'2')
                            current['trade'].discard(code)
                            time.sleep(SETTINGS.ws_subscribe_delay)
                        for code in trade_codes:
                            if code not in current['trade']:
                                send_sub(ws,trade_tr,code,'1')
                                current['trade'].add(code)
                                time.sleep(SETTINGS.ws_subscribe_delay)
                        for code in order_codes:
                            if code not in current['order']:
                                send_sub(ws,order_tr,code,'1')
                                current['order'].add(code)
                                time.sleep(SETTINGS.ws_subscribe_delay)
    
                        low=market.lower()
                        state[f'{low}_trade_subscribed']=len(current['trade'])
                        state[f'{low}_orderbook_subscribed']=len(current['order'])
                        state[f'ws_{low}']='connected' if market_open(market) else 'waiting_market_session'
                    ...
                    def refresh_subscriptions(ws):
                        while not refresh_stop.wait(30):
                            try:
                                ...
                                state['last_error']=f'통합 WS 구독 갱신: {e}'
                                log.warning('통합 WS 구독 갱신 실패: %s',e)
                                try: ws.close()
                    ...
                    def initial_subscribe(ws):
                        try:
                            ...
                            threading.Thread(
                                ...
                                args=(ws,),
                                daemon=True,
                                name='KIS-subscription-refresh'
                            ).start()
                            log.info(
                                '통합 WS 구독 요청 완료: KRX 체결 %s / 호가 %s, NXT 체결 %s / 호가 %s',
                                state.get('krx_trade_subscribed',0),state.get('krx_orderbook_subscribed',0),
                                state.get('nxt_trade_subscribed',0),state.get('nxt_orderbook_subscribed',0)
                            )
                        except Exception as e:
                            refresh_stop.set()
                            state['last_error']=f'초기 구독: {e}'
                            log.warning('초기 구독 실패: %s',e)
                            try:
                                ws.close()
                            except Exception:
                                pass
                    ...
                    def opened(ws):
                        ...
                        threading.Thread(
                            ...
                            args=(ws,),
                            daemon=True,
                            name='KIS-initial-subscribe'
                        ).start()
    
                    def message(ws,msg):
                        nonlocal duplicate_appkey
                        if isinstance(msg,(bytes,bytearray,memoryview)):
                            msg=bytes(msg).decode('utf-8',errors='ignore')
                            ...
                            try:
                                ...
                                if rt_cd not in ('0',''):
                                    ...
                                    if duplicate_error:
                                        ...
                                        state['ws_krx']='duplicate_appkey'
                                        state['ws_nxt']='duplicate_appkey'
                                        state['krx_last_error']=msg1
                                        state['nxt_last_error']=msg1
                                        log.error('통합 WS 앱키 중복 사용 감지: %s',msg1)
                                        refresh_stop.set()
                                        try: ws.close()
                                        except Exception: pass
                                        return
                                    log.warning('통합 WS 구독응답 오류: tr=%s, rt_cd=%s, msg=%s',tr_id,rt_cd,msg1)
                            except Exception as e:
                                log.debug('통합 WS JSON 처리 실패: %s',e)
                            return
    
                        parts=msg.split('|',3)
                        tr_id=parts[1] if len(parts)>=2 else ''
                        if tr_id==SETTINGS.krx_order_tr:
                            ob=self.parse_book(msg,'KRX',SETTINGS.krx_order_tr)
                            if ob:on_book(ob)
                            return
                        if tr_id==SETTINGS.nxt_order_tr:
                            ...
                            if ob:on_book(ob)
                            return
                        if tr_id==SETTINGS.krx_trade_tr:
                            for tick in self.parse_trade(msg,'KRX',names):on_tick(tick)
                            return
                        if tr_id==SETTINGS.nxt_trade_tr:
                            for tick in self.parse_trade(msg,'NXT',names):on_tick(tick)
    
                    def error(ws,e):
                        refresh_stop.set()
                        state['last_error']=f'통합 WS: {e}'
                        if market_open('KRX'):state['ws_krx']='error'
                        if market_open('NXT'):state['ws_nxt']='error'
                        log.warning('통합 WS 오류: %s',e)
    
                    def closed(ws,status,msg):
                        refresh_stop.set()
                        if state.get('ws_krx') not in ('duplicate_appkey','waiting_market_session','disabled'):
                            state['ws_krx']='closed'
                        if state.get('ws_nxt') not in ('duplicate_appkey','waiting_market_session','disabled'):
                            state['ws_nxt']='closed'
                        log.warning('통합 WS 종료: status=%s, msg=%s',status,msg)
                    ...
                    refresh_stop.set()
                    state['ws_krx']='failed'
                    state['ws_nxt']='failed' if SETTINGS.enable_nxt else 'disabled'
                    state['last_error']=f'통합 WS: {e}'
                    log.exception('통합 stream 실패')
                    time.sleep(retry)
                    retry=min(120,retry*2)
        finally:
            with self.stream_lock:
...
class App:
 ...
 def handle(self,text,chat):
  ...
  if cmd in ('/도움말','/help'):BOT.send('<b>명령어</b>\n/상태 /단타 /스윙 /예비후보 /시장 /성과\n/매수 종목 매수가 수량 [단타|스윙]\n/매도 종목 /보유 /보유리셋\n/관심등록 종목 /관심삭제 종목 /관심목록\n/호가 종목 /후보갱신',chat)
   ...
   if c and price>0 and qty>0:
    ...
    DATABASE.upsert('tracked_positions',{'stock_code':c,'stock_name':x.name,'market':'INTEGRATED','entry_price':price,'current_price':price,'quantity':qty,'position_status':'entered','updated_at':now().isoformat()},'stock_code')
 ...
 def start(self):
  ...
  if SETTINGS.kis_app_key and SETTINGS.kis_app_secret:
   threading.Thread(target=KIS_CLIENT.stream,args=(STATE.realtime_codes,STATE.names,self.on_tick,self.on_book,STATE.runtime),daemon=True,name='KIS-integrated-websocket').start()
  else:
   STATE.runtime['ws_krx']='disabled_missing_credentials'
...
if __name__=='__main__':
 threading.Thread(target=APP.start,daemon=True,name='app-initializer').start()
