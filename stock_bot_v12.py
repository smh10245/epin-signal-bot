...
log=logging.getLogger('v12.infra')
    ...
    def stream(self,market,codes_fn,names,on_tick,on_book,state):
        ...
        while True:
            ...
            try:
                ...
                subscribed_trade=set()
                subscribed_order=set()

                def sub(ws,tr,code,tr_type='1'):
                    payload={
                        'header':{
                            'approval_key':key,
                            'custtype':'P',
                ...
                def sync_subscriptions(ws):
                    trade_codes,order_codes=desired_codes()
                    new_trade=set(trade_codes);new_order=set(order_codes)
                    for code in list(subscribed_order-new_order):
                        sub(ws,order,code,'2');subscribed_order.discard(code);time.sleep(SETTINGS.ws_subscribe_delay)
                    for code in list(subscribed_trade-new_trade):
                        sub(ws,trade,code,'2');subscribed_trade.discard(code);time.sleep(SETTINGS.ws_subscribe_delay)
                    for code in trade_codes:
                        if code not in subscribed_trade:
                            sub(ws,trade,code);subscribed_trade.add(code);time.sleep(SETTINGS.ws_subscribe_delay)
                    for code in order_codes:
                        if code not in subscribed_order:
                            sub(ws,order,code);subscribed_order.add(code);time.sleep(SETTINGS.ws_subscribe_delay)
                    state[f'{market.lower()}_trade_subscribed']=len(subscribed_trade)
                    state[f'{market.lower()}_orderbook_subscribed']=len(subscribed_order)

                def refresh_subscriptions(ws):
                    while not refresh_stop.wait(30):
                        try:
                            sync_subscriptions(ws)
                        except Exception as e:
                            state[f'{market.lower()}_last_error']=str(e)
                            log.warning('%s WS 구독 갱신 실패: %s',market,e)
                            return
                ...
                def opened(ws):
                    ...
                    sync_subscriptions(ws)
                    threading.Thread(target=refresh_subscriptions,args=(ws,),daemon=True,name=f'{market}-subscription-refresh').start()
                    log.info(
...
log=logging.getLogger('v12.core')
...
class App:
 ...
 def start(self):
  ...
  if SETTINGS.kis_app_key and SETTINGS.kis_app_secret:
   threading.Thread(target=KIS_CLIENT.stream,args=('KRX',STATE.realtime_codes,STATE.names,self.on_tick,self.on_book,STATE.runtime),daemon=True,name='KRX-websocket').start()
   if SETTINGS.enable_nxt and SETTINGS.kis_env!='virtual':
    threading.Thread(target=KIS_CLIENT.stream,args=('NXT',STATE.realtime_codes,STATE.names,self.on_tick,self.on_book,STATE.runtime),daemon=True,name='NXT-websocket').start()
  else:
