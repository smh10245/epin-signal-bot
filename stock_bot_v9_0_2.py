...
# ============================================================
# V9.0.2 FINAL FEATURE OVERLAY
# - 거래대금/시총 기반 150~200개 후보군
# - KRX/NXT 체결 + 통합호가
# - 기영이/V자/눌림목/거래량/체결강도/호가 종합점수
# - 등급, TOP5, 추천취소, 추천강도 변화, 추천이력
# - 회사 안정성 필터(상장정보 + Supabase 선택 데이터)
# - 자동 성과 추적
# ============================================================

APP_VERSION = "9.0.2-final"

MAX_CANDIDATES = int(os.getenv("MAX_CANDIDATES", "200"))
KOSPI_CANDIDATES = int(os.getenv("KOSPI_CANDIDATES", "100"))
KOSDAQ_CANDIDATES = int(os.getenv("KOSDAQ_CANDIDATES", "50"))
REALTIME_SUBSCRIPTION_LIMIT = int(os.getenv("REALTIME_SUBSCRIPTION_LIMIT", "80"))
MIN_MARKET_CAP = float(os.getenv("MIN_MARKET_CAP", "100000000000"))
MIN_DAILY_VOLUME = int(os.getenv("MIN_DAILY_VOLUME", "20000"))
RECOMMEND_SCORE = float(os.getenv("RECOMMEND_SCORE", "78"))
CANCEL_SCORE = float(os.getenv("CANCEL_SCORE", "62"))
GRADE_CHANGE_NOTIFY = os.getenv("GRADE_CHANGE_NOTIFY", "true").lower() in ("1","true","yes","on")

KIS_KRX_ORDERBOOK_TR_ID = os.getenv("KIS_KRX_ORDERBOOK_TR_ID", "H0STASP0").strip()
KIS_NXT_ORDERBOOK_TR_ID = os.getenv("KIS_NXT_ORDERBOOK_TR_ID", "H0UNASP0").strip()

candidate_pool: Dict[str, Dict[str, Any]] = {}
orderbooks: Dict[Tuple[str, str], Dict[str, Any]] = {}
recommendation_state: Dict[str, Dict[str, Any]] = {}
recommendation_history: Deque[Dict[str, Any]] = deque(maxlen=2000)
latest_scores: Dict[str, Dict[str, Any]] = {}
ws_instances: Dict[str, websocket.WebSocketApp] = {}
ws_instance_lock = threading.Lock()
candidate_refresh_lock = threading.Lock()
fundamental_cache: Dict[str, Dict[str, Any]] = {}
market_risk_state: Dict[str, Any] = {"score": 50.0, "level": "보통", "updated_at": None}
...
def _manual_priority_codes() -> List[str]:
    codes: List[str] = []
    for code in manual_holdings:
    ...
    with watchlist_lock:
        for code in WATCHLIST:
            if code not in codes:
                codes.append(code)
    for code, state in recommendation_state.items():
        if state.get("active") and code not in codes:
            codes.append(code)
    return codes


def load_fundamental_filters() -> None:
    global fundamental_cache
    rows = store.select("company_fundamentals", {"select": "*"})
...
def company_is_stable(code: str, meta: Dict[str, Any]) -> Tuple[bool, List[str]]:
    reasons: List[str] = []
    marcap = _safe_num(meta.get("Marcap") or meta.get("market_cap"))
    volume = int(_safe_num(meta.get("Volume") or meta.get("volume")))
    if marcap and marcap < MIN_MARKET_CAP:
        reasons.append("시가총액 기준 미달")
    if volume and volume < MIN_DAILY_VOLUME:
        reasons.append("거래량 부족")

    f = fundamental_cache.get(code, {})
    if str(f.get("is_managed", "")).lower() in ("true", "1", "y", "yes"):
        reasons.append("관리종목")
    if str(f.get("is_halted", "")).lower() in ("true", "1", "y", "yes"):
        reasons.append("거래정지")
    if int(_safe_num(f.get("consecutive_loss_years"))) >= 3:
        reasons.append("지속 적자")
    liquidity = _safe_num(f.get("liquidity_score"), 100)
    if liquidity and liquidity < 30:
        reasons.append("유동성 부족")
    return (len(reasons) == 0), reasons


def refresh_candidate_pool(force_reconnect: bool = True) -> Dict[str, Dict[str, Any]]:
    global candidate_pool
    with candidate_refresh_lock:
        load_fundamental_filters()
        ...
        try:
            ...
            for _, r in df.iterrows():
                ...
                name = str(row.get("Name") or STOCK_MASTER.get(code) or code)
                ...
                if not amount:
                    amount = _safe_num(row.get("Close")) * _safe_num(row.get("Volume"))
                row["turnover"] = amount
                stable, filter_reasons = company_is_stable(code, row)
                row["stable"] = stable
                row["filter_reasons"] = filter_reasons
                if stable:
                    rows.append(row)
        except Exception as exc:
            logger.warning("후보군 종목목록 조회 실패: %s", exc)

        kospi = [x for x in rows if "KOSPI" in x.get("market","").upper()]
        kosdaq = [x for x in rows if "KOSDAQ" in x.get("market","").upper()]
        kospi.sort(key=lambda x: (x.get("turnover",0), _safe_num(x.get("Marcap"))), reverse=True)
        kosdaq.sort(key=lambda x: (x.get("turnover",0), _safe_num(x.get("Marcap"))), reverse=True)

        selected = kospi[:KOSPI_CANDIDATES] + kosdaq[:KOSDAQ_CANDIDATES]
        pool: Dict[str, Dict[str, Any]] = {x["code"]: x for x in selected if x.get("code")}

        # 보유/관심/추천 종목은 안정성 필터와 무관하게 항상 포함
        for code in _manual_priority_codes():
            pool.setdefault(code, {
                "code": code,
                "name": WATCHLIST.get(code) or STOCK_MASTER.get(code) or code,
                "market": "PRIORITY",
                "turnover": float("inf"),
                "stable": True,
                "priority": True,
        ...
        # 최대 200개로 제한하되 우선종목은 유지
        priority = _manual_priority_codes()
        ordered_codes = priority + [
            code for code, _ in sorted(
                pool.items(),
                key=lambda kv: _safe_num(kv[1].get("turnover")),
                reverse=True,
            ) if code not in priority
        ]
        ordered_codes = ordered_codes[:MAX_CANDIDATES]
        candidate_pool = {code: pool[code] for code in ordered_codes if code in pool}

        # WATCHLIST를 후보군으로 갱신
        with watchlist_lock:
            for code, meta in candidate_pool.items():
                WATCHLIST[code] = meta.get("name") or STOCK_MASTER.get(code) or code

        runtime["candidate_count"] = len(candidate_pool)
        runtime["candidate_refreshed_at"] = now_kst().isoformat()

        if force_reconnect:
            with ws_instance_lock:
                for ws in list(ws_instances.values()):
                    try:
                        ws.close()
                    except Exception:
                        pass
...
def _realtime_codes() -> List[str]:
    priority = _manual_priority_codes()
    scored = []
    for code, meta in candidate_pool.items():
        turnover = _safe_num(meta.get("turnover"))
        score = turnover
        if code in priority:
            score = float("inf")
        scored.append((score, code))
    scored.sort(reverse=True, key=lambda x: x[0])
    codes = priority + [c for _, c in scored if c not in priority]
    return codes[:REALTIME_SUBSCRIPTION_LIMIT]


def parse_orderbook_frame(message: str, market: str) -> Optional[Dict[str, Any]]:
    if not message or message.startswith("{"):
        return None
    parts = message.split("|", 3)
    if len(parts) < 4:
        return None
    tr_id = parts[1]
    if tr_id not in (KIS_KRX_ORDERBOOK_TR_ID, KIS_NXT_ORDERBOOK_TR_ID):
        return None
    fields = parts[3].split("^")
    if len(fields) < 45:
        return None
    code = str(fields[0]).zfill(6)
    asks = [_safe_num(x) for x in fields[3:13]]
    bids = [_safe_num(x) for x in fields[13:23]]
    ask_qty = [int(_safe_num(x)) for x in fields[23:33]]
    bid_qty = [int(_safe_num(x)) for x in fields[33:43]]
    total_ask = int(_safe_num(fields[43])) if len(fields) > 43 else sum(ask_qty)
    total_bid = int(_safe_num(fields[44])) if len(fields) > 44 else sum(bid_qty)
    ratio = total_bid / total_ask if total_ask > 0 else 0.0

    valid_ask = [(p, q) for p, q in zip(asks, ask_qty) if p > 0]
    valid_bid = [(p, q) for p, q in zip(bids, bid_qty) if p > 0]
    median_ask = sorted([q for _, q in valid_ask])[len(valid_ask)//2] if valid_ask else 0
    median_bid = sorted([q for _, q in valid_bid])[len(valid_bid)//2] if valid_bid else 0
    sell_walls = [(p, q) for p, q in valid_ask if median_ask and q >= median_ask * 3]
    buy_walls = [(p, q) for p, q in valid_bid if median_bid and q >= median_bid * 3]
    ...
    orderbooks[(market, code)] = result
    return result


def _recent_bars(code: str, market: str) -> List[MinuteBar]:
    return list(engine.history.get((market, code), []))


def _pattern_scores(code: str, market: str) -> Dict[str, float]:
    bars = _recent_bars(code, market)
    result = {"v": 0.0, "pullback": 0.0, "kiyoung": 0.0, "trend": 0.0, "volume": 0.0, "strength": 0.0}
    if len(bars) < 12:
        return result
    ...
    hi = max(highs[:-2]) if len(highs) > 2 else max(highs)
    lo = min(lows)
    decline = (lo / hi - 1) * 100 if hi else 0
    rebound = (latest.close / lo - 1) * 100 if lo else 0
    if decline <= -2 and rebound >= 1.2:
        result["v"] = min(100, abs(decline) * 12 + rebound * 12)

    ema5 = engine._ema(closes, 5)
    ema12 = engine._ema(closes, 12)
    if ema5 > ema12 and latest.close >= ema5:
        result["trend"] = min(100, 60 + (latest.close / ema5 - 1) * 1000)
    pullback_low = min(lows[-6:])
    prev_high = max(highs[-10:-1])
    depth = (pullback_low / prev_high - 1) * 100 if prev_high else 0
    if -5.5 <= depth <= -0.8 and latest.close >= max(highs[-6:-1]):
        result["pullback"] = 85

    avg_old = sum(volumes[-12:-3]) / max(1, len(volumes[-12:-3]))
    avg_new = sum(volumes[-3:]) / 3
    ratio = avg_new / avg_old if avg_old else 0
    result["volume"] = min(100, ratio * 40)
    result["strength"] = min(100, max(0, latest.trade_strength - 70))

    # 기영이 패턴: 일정 가격대 지지 반복 + 얕은 조정 + 거래량 재유입
    window = recent[-18:]
    support = sorted([b.low for b in window])[max(0, len(window)//4 - 1)]
    touches = sum(1 for b in window if support and abs(b.low / support - 1) <= 0.012)
    range_pct = (max(b.high for b in window) / min(b.low for b in window) - 1) * 100
    if touches >= 3 and range_pct <= 12 and ratio >= 1.25 and latest.close > support * 1.01:
        result["kiyoung"] = min(100, 45 + touches * 10 + ratio * 12)
    return result
...
def calculate_market_risk() -> Dict[str, Any]:
    changes: List[float] = []
    with watchlist_lock:
        codes = list(WATCHLIST.keys())[:50]
    for code in codes:
        try:
            out = kis.inquire_price(code)
            changes.append(_safe_num(out.get("prdy_ctrt")))
        except Exception:
            continue
    if not changes:
        return market_risk_state
    avg = sum(changes) / len(changes)
    negative_ratio = sum(1 for x in changes if x < 0) / len(changes)
    risk = 50 - avg * 8 + negative_ratio * 35
    risk = max(0, min(100, risk))
    level = "높음" if risk >= 70 else "보통" if risk >= 40 else "낮음"
    market_risk_state.update(score=risk, level=level, updated_at=now_kst().isoformat())
    return market_risk_state


def calculate_composite_score(code: str, market: str = "KRX") -> Dict[str, Any]:
    patterns = _pattern_scores(code, market)
    ob = orderbooks.get((market, code)) or orderbooks.get(("NXT", code)) or {}
    imbalance = _safe_num(ob.get("imbalance"))
    order_score = max(0, min(100, 50 + (imbalance - 1) * 35))
    meta = candidate_pool.get(code, {})
    stable, filter_reasons = company_is_stable(code, meta)
    stability_score = 100 if stable else 20
    risk = calculate_market_risk()
    risk_score = 100 - _safe_num(risk.get("score"), 50)

    weights = {
        "v": 0.16, "pullback": 0.14, "kiyoung": 0.16,
        "volume": 0.14, "strength": 0.10, "trend": 0.12,
        "order": 0.08, "stability": 0.06, "risk": 0.04,
    }
    components = dict(patterns)
    components["order"] = order_score
    ...
    reasons: List[str] = []
    labels = {
        "v":"V자 반등","pullback":"눌림목 재돌파","kiyoung":"기영이 패턴",
        "volume":"거래량 증가","strength":"체결강도","trend":"상승추세",
        "order":"호가 매수우위","stability":"회사 안정성","risk":"시장 위험도 양호"
    }
    for k, v in sorted(components.items(), key=lambda x: x[1], reverse=True):
        if v >= 65:
            reasons.append(f"{labels.get(k,k)} {v:.0f}점")
    if filter_reasons:
        reasons.extend(f"제외 사유: {x}" for x in filter_reasons)
    ...
    latest_scores[code] = result
    return result


def _persist_recommendation_event(event: Dict[str, Any]) -> None:
    recommendation_history.append(event)
    store.insert("recommendations", {
        "stock_code": event.get("code"),
        "stock_name": event.get("name"),
        "recommended_price": event.get("price"),
        "confidence_score": event.get("score"),
        "recommendation_time": event.get("timestamp"),
    })


def evaluate_recommendation_lifecycle(code: str, market: str, price: float) -> None:
    result = calculate_composite_score(code, market)
    state = recommendation_state.get(code)
    now_iso = now_kst().isoformat()

    if result["score"] >= RECOMMEND_SCORE and result["stable"]:
        if not state or not state.get("active"):
            recommendation_state[code] = {
                "active": True, "score": result["score"], "grade": result["grade"],
                "entry_price": price, "highest_price": price, "lowest_price": price,
                "started_at": now_iso, "reasons": result["reasons"],
            }
            with watchlist_lock:
                WATCHLIST[code] = result["name"]
            event = {
                "event":"recommend","code":code,"name":result["name"],"price":price,
                "score":result["score"],"grade":result["grade"],"timestamp":now_iso,
                "reasons":result["reasons"],
            }
            _persist_recommendation_event(event)
            telegram.send(
                f"🚀 <b>추천 발생</b>\n\n"
                f"{html.escape(result['name'])} ({code})\n"
                f"{result['stars']} · <b>{result['grade']}</b> · {result['score']:.1f}점\n"
                f"현재가 {price:,.0f}원\n"
                + "\n".join(f"• {html.escape(x)}" for x in result["reasons"])
                + "\n\n※ 실제 매수 후 반드시 /매수 종목명 매수가 수량 입력"
            )
        else:
            state["highest_price"] = max(_safe_num(state.get("highest_price")), price)
            state["lowest_price"] = min(_safe_num(state.get("lowest_price"), price), price)
            if GRADE_CHANGE_NOTIFY and result["grade"] != state.get("grade"):
                telegram.send(
                    f"🔄 <b>추천 강도 변화</b>\n\n"
                    f"{html.escape(result['name'])} ({code})\n"
                    f"{state.get('grade')} → <b>{result['grade']}</b>\n"
                    f"{state.get('score',0):.1f}점 → {result['score']:.1f}점"
                )
            state["grade"] = result["grade"]
            state["score"] = result["score"]
    elif state and state.get("active") and result["score"] < CANCEL_SCORE:
        state["active"] = False
        state["cancelled_at"] = now_iso
        entry = _safe_num(state.get("entry_price"))
        max_ret = (_safe_num(state.get("highest_price")) / entry - 1) * 100 if entry else 0
        min_ret = (_safe_num(state.get("lowest_price")) / entry - 1) * 100 if entry else 0
        event = {
            "event":"cancel","code":code,"name":result["name"],"price":price,
            "score":result["score"],"grade":result["grade"],"timestamp":now_iso,
            "max_return":max_ret,"min_return":min_ret,
        }
        recommendation_history.append(event)
        telegram.send(
            f"⛔ <b>추천 취소 · 매수 금지</b>\n\n"
            f"{html.escape(result['name'])} ({code})\n"
            f"점수 {result['score']:.1f}점 · {result['grade']}\n"
            f"추천 조건이 무너졌습니다."
        )
...
def process_tick(tick: Tick) -> None:
    ...
    if not completed:
        return
    signals = engine.on_bar(completed)
    # 추천 신호는 실제 보유로 자동 등록하지 않음
    for signal in signals:
        runtime["last_signal_at"] = signal.timestamp.isoformat()
        runtime["signals_today"] += 1
        if signal.signal_type == "buy_valid":
            evaluate_recommendation_lifecycle(signal.code, signal.market, signal.price)
            persist_signal(signal)
        else:
            telegram.send(format_signal(signal))
            persist_signal(signal)
    # 신호가 없어도 활성 추천은 계속 재평가
    if tick.code in recommendation_state:
        evaluate_recommendation_lifecycle(tick.code, tick.market, completed.close)


def run_ws(market: str, tr_id: str) -> None:
    state_key = "ws_krx" if market == "KRX" else "ws_nxt"
    retry_seconds = 10
    order_tr = KIS_KRX_ORDERBOOK_TR_ID if market == "KRX" else KIS_NXT_ORDERBOOK_TR_ID

    while True:
        if not market_session_open(market):
            runtime[state_key] = "waiting_market_session"
            time.sleep(min(seconds_until_session(market), 900))
            continue
        opened_at = None
        try:
            approval_key = kis.get_approval_key()
            ...
            def _send_sub(ws: websocket.WebSocketApp, sub_tr: str, code: str) -> None:
                ws.send(json.dumps({
                    "header": {
                        "approval_key": approval_key,
                        "custtype": "P",
                    ...
                    "body": {"input": {"tr_id": sub_tr, "tr_key": code}},
                }))
            ...
            def on_open(ws: websocket.WebSocketApp) -> None:
                ...
                codes = _realtime_codes()
                runtime[f"{market.lower()}_subscribed"] = len(codes)
                for code in codes:
                    _send_sub(ws, tr_id, code)
                    time.sleep(0.04)
                    _send_sub(ws, order_tr, code)
                    time.sleep(0.04)
            ...
            def on_message(ws: websocket.WebSocketApp, message: str) -> None:
                if message.startswith("{"):
                    try:
                        ...
                        pass
                    return
                ob = parse_orderbook_frame(message, market)
                if ob is not None:
                    return
                for tick in parse_trade_frame(message, market):
                    process_tick(tick)
            ...
            def on_error(ws: websocket.WebSocketApp, error: Any) -> None:
                runtime[state_key] = "error"
                runtime["kis_last_error"] = str(error)
                logger.warning("%s WS 오류: %s", market, error)

            def on_close(ws: websocket.WebSocketApp, status: Any, msg: Any) -> None:
                runtime[state_key] = "closed"
                with ws_instance_lock:
                    ws_instances.pop(market, None)

            websocket.WebSocketApp(
                kis.ws_url, on_open=on_open, on_message=on_message,
                on_error=on_error, on_close=on_close,
            ).run_forever(ping_interval=30, ping_timeout=10)

            connected = time.monotonic() - opened_at if opened_at else 0
            retry_seconds = 10 if connected >= 300 else min(retry_seconds * 2, 300)
        except Exception as exc:
            runtime[state_key] = "failed"
            runtime["kis_last_error"] = str(exc)
            logger.exception("%s WS 실행 실패", market)
            retry_seconds = min(retry_seconds * 2, 300)
        if market_session_open(market):
            time.sleep(retry_seconds)


def orderbook_text(query: str) -> str:
    code, name = normalize_code_or_name(query)
    if not code:
        return "종목을 찾지 못했습니다."
    ob = orderbooks.get(("NXT", code)) or orderbooks.get(("KRX", code))
    if not ob:
        return f"{html.escape(name or code)}의 호가 데이터가 아직 없습니다."
    lines = [
        f"📚 <b>{html.escape(name or code)} 호가</b>",
        f"시장: {ob['market']}",
        f"총 매수잔량: {ob['total_bid']:,}",
        f"총 매도잔량: {ob['total_ask']:,}",
        f"매수/매도 잔량비: <b>{ob['imbalance']:.2f}배</b>",
        "",
        "매도호가",
    ]
    for i, (p, q) in enumerate(zip(ob["asks"][:5], ob["ask_qty"][:5]), 1):
        lines.append(f"{i}호가 {p:,.0f}원 · {q:,}주")
    lines.append("")
    lines.append("매수호가")
    for i, (p, q) in enumerate(zip(ob["bids"][:5], ob["bid_qty"][:5]), 1):
        lines.append(f"{i}호가 {p:,.0f}원 · {q:,}주")
    if ob["sell_walls"]:
        lines.append("")
        lines.append("⚠️ 매도벽: " + ", ".join(f"{p:,.0f}원({q:,})" for p,q in ob["sell_walls"]))
    if ob["buy_walls"]:
        lines.append("🛡 매수벽: " + ", ".join(f"{p:,.0f}원({q:,})" for p,q in ob["buy_walls"]))
    return "\n".join(lines)


def top5_text() -> str:
    codes = list(candidate_pool.keys()) or list(WATCHLIST.keys())
    rows = []
    for code in codes[:MAX_CANDIDATES]:
        try:
            rows.append(calculate_composite_score(code, "KRX"))
        except Exception:
            continue
    rows.sort(key=lambda x: x["score"], reverse=True)
    lines = ["🏆 <b>오늘의 TOP5</b>", f"시장 위험도: {market_risk_state['level']}", ""]
    for i, r in enumerate(rows[:5], 1):
        lines.extend([
            f"{i}. <b>{html.escape(r['name'])}</b> ({r['code']})",
            f"   {r['stars']} · {r['grade']} · {r['score']:.1f}점",
            "   " + " / ".join(r["reasons"][:3]),
        ])
    return "\n".join(lines)


def recommendation_history_text(query: str) -> str:
    code, name = normalize_code_or_name(query)
    if not code:
        return "종목을 찾지 못했습니다."
    rows = [x for x in recommendation_history if x.get("code") == code]
    if not rows:
        return f"{html.escape(name or code)} 추천 이력이 없습니다."
    lines = [f"🗂 <b>{html.escape(name or code)} 추천 이력</b>", ""]
    for row in rows[-10:]:
        event = "추천" if row.get("event") == "recommend" else "추천취소"
        lines.append(
            f"• {row.get('timestamp','-')} · {event} · "
            f"{row.get('score',0):.1f}점 · {row.get('price',0):,.0f}원"
        )
    return "\n".join(lines)


def performance_auto_text() -> str:
    completed = [x for x in recommendation_history if x.get("event") == "cancel"]
    active = [x for x in recommendation_state.values() if x.get("active")]
    if not completed and not active:
        return "📈 아직 누적된 추천 성과가 없습니다."
    wins = sum(1 for x in completed if _safe_num(x.get("max_return")) >= 2)
    losses = sum(1 for x in completed if _safe_num(x.get("min_return")) <= -3)
    avg_max = sum(_safe_num(x.get("max_return")) for x in completed) / len(completed) if completed else 0
    hit = wins / len(completed) * 100 if completed else 0
    return (
        "📈 <b>자동 성과 요약</b>\n\n"
        f"완료 추천: {len(completed)}건\n"
        f"활성 추천: {len(active)}건\n"
        f"성공: {wins}건\n"
        f"손절권 진입: {losses}건\n"
        f"적중률: {hit:.1f}%\n"
        f"평균 최대수익률: {avg_max:+.2f}%"
    )
...
def status_text() -> str:
    return (
        f"🤖 <b>뽕실로봇 V{APP_VERSION}</b>\n\n"
        f"KIS 환경: <b>{html.escape(KIS_ENV)}</b>\n"
        f"KIS 인증: {runtime.get('kis_auth')}\n"
        f"Supabase: {runtime.get('supabase')}\n"
        f"KRX WS: {runtime.get('ws_krx')}\n"
        f"NXT WS: {runtime.get('ws_nxt')}\n"
        f"후보군: {len(candidate_pool)}개\n"
        f"KRX 구독: {runtime.get('krx_subscribed',0)}개\n"
        f"NXT 구독: {runtime.get('nxt_subscribed',0)}개\n"
        f"보유종목: {len(manual_holdings)}개\n"
        f"활성 추천: {sum(1 for x in recommendation_state.values() if x.get('active'))}개\n"
        f"시장 위험도: {market_risk_state['level']} ({market_risk_state['score']:.0f})\n"
        f"마지막 체결: {runtime.get('last_tick_at') or '-'}\n\n"
        f"자동주문 기능 없음"
    )
...
def handle_command(text: str, chat_id: str) -> None:
    ...
    if cmd in ("/도움말", "/help"):
        ...
        telegram.send(top5_text(), chat_id)
    elif cmd == "/호가":
        if len(parts) < 2:
            telegram.send("사용법: /호가 종목명(또는 코드)", chat_id)
        else:
            telegram.send(orderbook_text(" ".join(parts[1:])), chat_id)
    elif cmd == "/기록":
        if len(parts) < 2:
            telegram.send("사용법: /기록 종목명(또는 코드)", chat_id)
        else:
            telegram.send(recommendation_history_text(" ".join(parts[1:])), chat_id)
    elif cmd == "/후보갱신":
        refresh_candidate_pool(force_reconnect=True)
        telegram.send(f"✅ 후보군 {len(candidate_pool)}개 갱신 완료", chat_id)
    elif cmd == "/성과":
        telegram.send(performance_auto_text(), chat_id)
    elif cmd == "/추천":
        telegram.send(top5_text(), chat_id)
    else:
        _original_handle_command(text, chat_id)


def candidate_manager_loop() -> None:
    while True:
        try:
            refresh_candidate_pool(force_reconnect=True)
        except Exception as exc:
            logger.warning("후보군 자동 갱신 실패: %s", exc)
        time.sleep(600)


_original_scheduler_loop = scheduler_loop


def scheduler_loop() -> None:
    threading.Thread(target=candidate_manager_loop, daemon=True, name="candidate-manager").start()
    sent: Dict[str, str] = {}
    while True:
        now = now_kst()
        today = now.date().isoformat()
        if now.weekday() < 5:
            if now.hour == 7 and 30 <= now.minute < 35 and sent.get("us") != today:
                ...
                sent["us"] = today
            if now.hour == 8 and 20 <= now.minute < 25 and sent.get("pre") != today:
                refresh_candidate_pool(force_reconnect=False)
                telegram.send(top5_text())
                sent["pre"] = today
            if now.hour == 15 and 45 <= now.minute < 50 and sent.get("close") != today:
                telegram.send(performance_auto_text())
                sent["close"] = today
            if now.hour == 20 and 5 <= now.minute < 10 and sent.get("nxt") != today:
                telegram.send("🌙 <b>NXT 마감 브리핑</b>\n\n" + top5_text())
                sent["nxt"] = today
        time.sleep(20)
...
def startup() -> None:
    ...
    if ENABLE_TELEGRAM_POLLING:
        ...
        runtime["telegram"] = "send_only"
    threading.Thread(target=scheduler_loop, daemon=True, name="scheduler").start()
    threading.Thread(target=run_ws, args=("KRX", KIS_KRX_TRADE_TR_ID), daemon=True, name="ws-krx").start()
    if KIS_ENV != "virtual" or ENABLE_NXT_ON_VIRTUAL:
        threading.Thread(target=run_ws, args=("NXT", KIS_NXT_TRADE_TR_ID), daemon=True, name="ws-nxt").start()
    else:
        runtime["ws_nxt"] = "skipped_virtual"
...
# ============================================================
# V9.0.2 FINAL HOTFIX
# - /추천 TOP5 계산 중 텔레그램 명령 수신 멈춤 해결
# - 시장위험도 중복 REST 호출 제거
# - 무거운 명령 비동기 처리
# - 명령 처리 예외 시 사용자에게 오류 회신
# ============================================================

APP_VERSION = "9.0.2-final-hotfix"

_market_risk_cache = {
    "value": dict(market_risk_state),
    "expires_at": 0.0,
}
_heavy_command_lock = threading.Lock()


def calculate_market_risk_cached(force: bool = False) -> Dict[str, Any]:
    now_mono = time.monotonic()
    if not force and now_mono < float(_market_risk_cache.get("expires_at", 0)):
        return dict(_market_risk_cache["value"])

    changes: List[float] = []
    # 최대 20종목만 확인하고 5분간 캐시
    with watchlist_lock:
        codes = list(WATCHLIST.keys())[:20]
    ...
    if changes:
        avg = sum(changes) / len(changes)
        negative_ratio = sum(1 for x in changes if x < 0) / len(changes)
        risk = max(0, min(100, 50 - avg * 8 + negative_ratio * 35))
        level = "높음" if risk >= 70 else "보통" if risk >= 40 else "낮음"
        market_risk_state.update(
            score=risk,
            level=level,
            updated_at=now_kst().isoformat(),
        )

    _market_risk_cache["value"] = dict(market_risk_state)
    _market_risk_cache["expires_at"] = now_mono + 300
    return dict(market_risk_state)


def calculate_composite_score_fast(
    code: str,
    market: str = "KRX",
    ...
    stable, filter_reasons = company_is_stable(code, meta)
    stability_score = 100 if stable else 20
    risk = risk or calculate_market_risk_cached()
    risk_score = 100 - _safe_num(risk.get("score"), 50)
...
def top5_text_fast() -> str:
    # 장 전에는 실시간 봉이 없으므로 최대 60개까지만 계산
    codes = list(candidate_pool.keys()) or list(WATCHLIST.keys())
    codes = codes[:60]
    risk = calculate_market_risk_cached()

    rows: List[Dict[str, Any]] = []
    for code in codes:
        try:
            rows.append(calculate_composite_score_fast(code, "KRX", risk))
        except Exception as exc:
            logger.debug("TOP5 점수 계산 실패 %s: %s", code, exc)

    rows.sort(key=lambda x: x["score"], reverse=True)
    lines = [
        "🏆 <b>오늘의 TOP5</b>",
        f"시장 위험도: {risk.get('level', '보통')}",
        "",
    ]
    if not rows:
        lines.append("점수 계산 가능한 종목이 아직 없습니다.")
        return "\n".join(lines)

    for i, row in enumerate(rows[:5], 1):
        reasons = " / ".join(row["reasons"][:3]) or "장중 데이터 축적 대기"
        lines.extend(
            [
                f"{i}. <b>{html.escape(row['name'])}</b> ({row['code']})",
                f"   {row['stars']} · {row['grade']} · {row['score']:.1f}점",
...
def _run_heavy_command(command: str, chat_id: str) -> None:
    if not _heavy_command_lock.acquire(blocking=False):
        telegram.send(
            "⏳ 다른 분석 작업이 진행 중입니다. 잠시 후 다시 시도해 주세요.",
            chat_id,
        )
        return
    try:
        if command in ("/추천", "/TOP5"):
            telegram.send(top5_text_fast(), chat_id)
        elif command == "/후보갱신":
            pool = refresh_candidate_pool(force_reconnect=True)
            telegram.send(f"✅ 후보군 {len(pool)}개 갱신 완료", chat_id)
    except Exception as exc:
        logger.exception("무거운 명령 처리 실패: %s", command)
        telegram.send(
            f"⚠️ {html.escape(command)} 처리 중 오류가 발생했습니다.\n"
            f"Render 로그에서 최신 오류를 확인해 주세요.",
            chat_id,
        )
    finally:
        _heavy_command_lock.release()


_previous_handle_command = handle_command
...
def handle_command(text: str, chat_id: str) -> None:
    try:
        ...
        if cmd in ("/추천", "/TOP5", "/후보갱신"):
            telegram.send(
                "⏳ 분석을 시작했습니다. 완료되면 결과를 보내드릴게요.",
                chat_id,
            )
            threading.Thread(
                target=_run_heavy_command,
                args=(cmd, chat_id),
                daemon=True,
                name=f"telegram-heavy-{cmd.lstrip('/')}",
            ).start()
            return

        _previous_handle_command(text, chat_id)
    except Exception as exc:
        logger.exception("텔레그램 명령 처리 오류: %s", text)
        telegram.send(
            "⚠️ 명령 처리 중 오류가 발생했습니다. 입력 형식을 확인해 주세요.",
            chat_id,
        )


def poll_forever_hotfix(self) -> None:
    if not TELEGRAM_TOKEN:
        return
    while True:
        try:
            ...
            for update in response.json().get("result", []):
                ...
                chat_id = str((message.get("chat") or {}).get("id") or "")
                if text and chat_id:
                    # 각 명령을 별도 스레드에서 처리해 폴링이 멈추지 않게 함
                    threading.Thread(
            ...
            if status == 409:
                runtime["telegram"] = "send_only_conflict_409"
                logger.error("텔레그램 409 충돌: 다른 프로세스가 같은 봇을 폴링 중입니다.")
                return
            logger.warning("텔레그램 폴링 HTTP 오류: %s", exc)
            time.sleep(10)
        except Exception as exc:
...
Telegram.poll_forever = poll_forever_hotfix
