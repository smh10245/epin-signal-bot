...
def normalize_code_or_name(token: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    if token.isdigit():
        code = token.zfill(6)
        return code, WATCHLIST.get(code, code)
    for code, name in WATCHLIST.items():
        if token.replace(" ", "").lower() == name.replace(" ", "").lower():
            return code, name
    return None, None
...
def handle_command(text: str, chat_id: str) -> None:
    ...
    if cmd in ("/도움말", "/help"):
        ...
        if len(parts) < 4:
            telegram.send(
                "사용법: /매수 종목코드 매수가 수량\n"
                "예: /매수 005930 70000 10",
                chat_id,
            )
            return
        code, name = normalize_code_or_name(parts[1])
        if not code or not name:
            telegram.send("종목코드 6자리로 입력하세요.", chat_id)
            return
        ...
        if len(parts) < 2:
            telegram.send("사용법: /매도 종목코드", chat_id)
            return
        code, name = normalize_code_or_name(parts[1])
        if not code:
        ...
        if len(parts) < 3:
            telegram.send("사용법: /관심등록 종목코드 종목명", chat_id)
            return
        code = parts[1].zfill(6)
        name = " ".join(parts[2:])
        with watchlist_lock:
            WATCHLIST[code] = name
        telegram.send(f"✅ {html.escape(name)} ({code}) 감시목록 등록", chat_id)
    elif cmd == "/관심삭제":
        if len(parts) < 2:
            telegram.send("사용법: /관심삭제 종목코드", chat_id)
            return
        code = parts[1].zfill(6)
        if code in manual_holdings:
            telegram.send("보유종목은 /매도 명령으로 먼저 종료하세요.", chat_id)
            return
        with watchlist_lock:
            removed = WATCHLIST.pop(code, None)
        telegram.send(f"✅ {html.escape(removed or code)} 감시목록 삭제", chat_id)
    else:
        telegram.send(help_text(), chat_id)
