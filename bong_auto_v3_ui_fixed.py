...
class Settings:
    version:str='3.1.1-auto-ui-fixed'
...
class State:
    ...
    def _load_listing_cache(self):
        try:
            ...
            meta=d.get('meta') or {}
            if names and meta:
                log.warning('실시간 종목 목록 대신 마지막 정상 캐시 %s개를 사용합니다.',len(names))
                return names,meta
        except Exception as e:
    ...
    def load(self):
            ...
            for attempt in range(1,4):
                try:
                    ...
                    if df is not None and not df.empty:
                        log.info('%s 종목 목록 로드 성공: %s개',market,len(df))
                        break
                    raise RuntimeError('빈 데이터')
                except Exception as e:
                    errors.append(f'{market} {attempt}/3: {e}')
                    log.warning('%s 종목 목록 로드 실패 %s/3: %s',market,attempt,e)
        ...
        if names and meta:
            with self.lock:
                self.names=names
                self.meta=meta
                self.runtime['last_error']='종목목록 실시간 로드 실패 → 캐시 사용'
            return

        raise RuntimeError(
            'KOSPI/KOSDAQ 종목 목록 로드와 캐시 복원 모두 실패: '
            + ' | '.join(errors[-4:])
        )
    def refresh(self):
...
class App:
 ...
 def start(self):
  ...
  BOT.send(
   ...
   f'종목목록 KOSPI+KOSDAQ 분리로딩 · 실패시 캐시 복원\n'
