...
class Brain:
    ...
    def evaluate_close(self,c):
        ...
        df=self.daily_history(c)
        bars=list(STATE.bars.get(('NXT',c),[]))
...
class App:
 ...
 def close_candidates(self,limit=None):
  """현재 후보 중 종가배팅 점수 상위 종목을 계산한다."""
  limit=limit or SETTINGS.close_limit
  rows=[]
  for code in list(STATE.candidates)[:100]:
   try:
    c=BRAIN.evaluate_close(code)
    if c.score>0 and c.stage!='데이터부족':
     rows.append(c)
   except Exception as e:
    log.debug('종배 계산 실패 %s: %s',code,e)
 ...
 def close_rank_text(self,limit=None):
  ...
  for i,c in enumerate(rows,1):
   ...
   lines += [
    f'{i}. <b>{html.escape(c.name)}</b> ({c.code}) · {c.stage}',
    f'   현재 {c.price:,.0f}원 · 점수 {c.score:.1f} · 신뢰 {c.reliability:.1f}%',
    f'   근거: {html.escape(why)}',
    f'   손절 {c.stop:,.0f}원 · 1차 {c.target1:,.0f}원 · 주의: {html.escape(caution)}',''
   ]
  return '\n'.join(lines)

 def save_close_picks(self,rows):
  try:
   payload={'date':str(now().date()),'saved_at':now().isoformat(),'picks':[
    {'code':c.code,'name':c.name,'price':c.price,'score':c.score,'reliability':c.reliability,
     'stop':c.stop,'target1':c.target1,'target2':c.target2,'stage':c.stage}
    for c in rows]}
 ...
 def load_close_picks(self):
  try:
   ...
   log.warning('종배 후보 읽기 실패: %s',e);return []

 def run_close_scan(self,stage,send=False,chat=None):
  self.close_scan_stage=stage
  rows=self.close_candidates(SETTINGS.close_limit if stage=='최종' else 10)
  self.close_picks=rows
  if stage=='최종':
   self.save_close_picks(rows)
  if send:
   BOT.send(self.close_rank_text(SETTINGS.close_limit if stage=='최종' else 10),chat)
  return rows

 def close_result_text(self):
  picks=self.load_close_picks()
  if not picks:return '📭 저장된 종가배팅 추천이 없습니다.'
  lines=['📈 <b>종가배팅 저장 내역</b>','']
  for x in picks:
   code=x.get('code'); name=x.get('name',code); rec=num(x.get('price'))
   current=0
   tick=STATE.ticks.get(('NXT',code))
   if tick:current=tick.price
   if current<=0:
    try:current=num(KIS_CLIENT.price(code).get('stck_prpr'))
    except Exception:current=0
   rate=pct(current,rec) if current and rec else 0
   lines.append(f"• <b>{html.escape(name)}</b> ({code})\n  추천 {rec:,.0f}원 · 현재 {current:,.0f}원 · {rate:+.2f}%")
  return '\n\n'.join(lines)
 ...
 def tomorrow_candidates(self,limit=5):
  ...
  rows=[]
  for code in list(STATE.candidates)[:max(SETTINGS.swing_scan_limit,80)]:
   try:
    ...
    reliability=max(day.reliability,swing.reliability)
    combined=day_score*.45+swing_score*.55+reliability*.10
    if combined<=0:
     continue
    ...
    rows.append((combined,reliability,code,kind,reasons,day_score,swing_score))
   except Exception as e:
    log.debug('익일 후보 계산 실패 %s: %s',code,e)
  rows.sort(reverse=True)
  result=[]
  for _,rel,code,kind,reasons,ds,ss in rows[:limit]:
   result.append({
    'code':code,
  ...
  if picks:
   ...
   for idx,x in enumerate(picks,1):
    ...
    blocks.append(
     ...
     f"   단타 {x['day_score']:.1f} · 스윙 {x['swing_score']:.1f} · 신뢰 {x['reliability']:.1f}%\n"
     f"   근거: {html.escape(why)}"
    )
   pick_text='\n\n'.join(blocks)
  else:
 ...
 def scheduler(self):
  ...
  while True:
   n=now();d=str(n.date())
   if n.weekday()<5:
    if n.hour==7 and 30<=n.minute<60 and sent.get('m')!=d:
     threading.Thread(target=lambda:BOT.send(self.morning_compare_text()),daemon=True,name='morning-compare').start();sent['m']=d
    if n.hour==14 and n.minute>=30 and sent.get('close1')!=d:
     threading.Thread(target=self.run_close_scan,args=('1차',False),daemon=True,name='close-scan-1430').start();sent['close1']=d
    if n.hour==15 and n.minute>=0 and sent.get('close2')!=d:
     threading.Thread(target=self.run_close_scan,args=('2차',False),daemon=True,name='close-scan-1500').start();sent['close2']=d
    if n.hour==15 and n.minute>=20 and sent.get('close_final')!=d:
     threading.Thread(target=self.run_close_scan,args=('최종',True,None),daemon=True,name='close-scan-final').start();sent['close_final']=d
    if n.hour==15 and 45<=n.minute<60 and sent.get('c')!=d:
     BOT.send(f'📊 <b>장마감 브리핑</b>\n오늘 추천 {len(self.recs)}건\n\n'+self.close_rank_text());sent['c']=d
    if n.hour==20 and n.minute>=0 and sent.get('n')!=d:BOT.send('🌙 <b>NXT 마감·익일 준비</b>\n\n'+self.close_result_text()+'\n\n'+self.rank('단타',5));sent['n']=d
    if n.hour==22 and sent.get('night')!=d:
     threading.Thread(target=self.send_night_briefing,daemon=True,name='night-briefing').start();sent['night']=d
