...
class Settings:
    version: str = "5.4.1 Myeongha Structure Target + Manual V Finder"
...
class App:
    ...
    def _yf_daily_chunk(self,pairs):
        ...
        try:
            d=yf.download(symbols,period="6mo",interval="1d",group_by="ticker",auto_adjust=False,threads=True,progress=False)
        except Exception as e:
    ...
    def run_v_finder(self,chat):
        ...
        def job():
            try:
                BOT.send("🔎 <b>VIP 반복 V자 종목발굴 시작</b>\n2만원 이상 · 반복 지지/반등 · 거래대금 · 저점 안정성 · 현재 위치를 실제 일봉으로 계산합니다.\n실시간 VIP 엔진과 분리된 별도 작업입니다.",chat)
                ...
                results=[];chunk_size=80;total=len(universe)
                for start in range(0,total,chunk_size):
                    chunk=universe[start:start+chunk_size]
                    pairs=[(c,mk) for c,mk,_ in chunk]
                    self.finder_progress=f"{min(start+len(chunk),total)}/{total}"
                ...
                if not self.finder_last_results:
                    BOT.send("🔎 종목발굴 완료\n현재 조건을 충분히 충족한 반복 V자 후보가 없습니다. 조건을 억지로 완화해 종목을 만들지 않았습니다.",chat,buttons=self.main_buttons());return
                ...
                for i,x in enumerate(self.finder_last_results,1):
                    ...
                    lines.append(
                        ...
                        f"현재 {x['price']:,.0f}원 · {pos}\n"
                        f"반복 진입관찰 {x['support_low']:,.0f}~{x['support_high']:,.0f}원\n"
                        f"과거구조 익절후보 {x['exit_low']:,.0f}~{x['exit_high']:,.0f}원\n"
                        f"지지 {x['touches']}회 · 5%+ 반등성공 {x['success_rate']*100:.0f}% · 중앙반등 {x['median_rebound_pct']:.1f}%"
                    )
                BOT.send("\n".join(lines),chat,buttons=self.main_buttons())
            except Exception as e:
                log.exception("V finder failed");BOT.send(f"⚠️ 종목발굴 오류: {html.escape(str(e)[:180])}",chat,buttons=self.main_buttons())
            finally:
