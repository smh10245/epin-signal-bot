...
from us_paper_trader import USPaperService
...
def normalize_intraday_bars(rows, trading_date=None):
    ...
    if trading_date is None:
        # 주말/휴일/재배포 후에는 "오늘"이 거래일이 아닐 수 있다.
        # 입력된 분봉 중 가장 최근 거래일을 자동 선택하고,
        # 장중에는 현재 틱이 가장 최신 날짜이므로 오늘 데이터가 자연스럽게 선택된다.
        trading_date=max(
            (kst_dt(b.minute).date() for b in (rows or []) if getattr(b,"minute",None) is not None),
            default=now().date(),
        )
    merged={}
    for b in rows or []:
...
class Settings:
    version: str = "5.9.0 Myeongha Dual Market Paper"
    ...
    # 08:00~20:00 NXT 전체 당일 흐름(최대 약 721분)을 보존한다.
    intraday_max_bars: int = env_int("INTRADAY_MAX_BARS", 780)
    ...
    domestic_sell_tax_pct: float = env_float("DOMESTIC_SELL_TAX_PCT", 0.20)
    ...
    paper_slippage_pct: float = env_float("PAPER_SLIPPAGE_PCT", 0.03)
    paper_max_positions: int = env_int("PAPER_MAX_POSITIONS", 3)
    paper_max_position_pct: float = env_float("PAPER_MAX_POSITION_PCT", 25.0)
    paper_max_daily_loss_pct: float = env_float("PAPER_MAX_DAILY_LOSS_PCT", 2.0)
    paper_min_entry_score: float = env_float("PAPER_MIN_ENTRY_SCORE", 72.0)
    paper_max_slippage_pct: float = env_float("PAPER_MAX_SLIPPAGE_PCT", 0.50)
    paper_latency_seconds: float = env_float("PAPER_LATENCY_SECONDS", 2.0)
    paper_max_minute_participation_pct: float = env_float("PAPER_MAX_MINUTE_PARTICIPATION_PCT", 3.0)
    paper_max_book_participation_pct: float = env_float("PAPER_MAX_BOOK_PARTICIPATION_PCT", 10.0)
    paper_min_fill_ratio: float = env_float("PAPER_MIN_FILL_RATIO", 0.25)
    paper_season_close_time: str = os.getenv("PAPER_SEASON_CLOSE_TIME", "19:50").strip()
    # KR Paper strategy tuning - explicit Render environment controls.
    paper_risk_per_trade_pct: float = env_float("PAPER_RISK_PER_TRADE_PCT", 0.50)
    paper_min_risk_reward: float = env_float("PAPER_MIN_RISK_REWARD", 1.60)
    paper_early_entry_fraction: float = env_float("PAPER_EARLY_ENTRY_FRACTION", 0.45)
    paper_breakeven_r: float = env_float("PAPER_BREAKEVEN_R", 1.0)
    paper_trail_start_r: float = env_float("PAPER_TRAIL_START_R", 1.5)
    paper_weak_confirm_seconds: int = env_int("PAPER_WEAK_CONFIRM_SECONDS", 60)
    # US Paper Trading - independent low-load engine.
    us_paper_enabled: bool = env_bool("US_PAPER_ENABLED", True)
    us_paper_universe: str = os.getenv(
        "US_PAPER_UNIVERSE",
        "AAPL,MSFT,NVDA,AMZN,META,GOOGL,TSLA,AMD,AVGO,NFLX,PLTR,CRM,ORCL,MU,QCOM,ARM,INTC,JPM,BAC,GS,XOM,CVX,LLY,UNH,COST,WMT,HD,MCD,CAT,GE,BA"
    ).strip()
    us_paper_candidate_limit: int = env_int("US_PAPER_CANDIDATE_LIMIT", 12)
    us_paper_scan_seconds: int = env_int("US_PAPER_SCAN_SECONDS", 300)
    us_paper_focus_seconds: int = env_int("US_PAPER_FOCUS_SECONDS", 60)
    us_paper_min_price: float = env_float("US_PAPER_MIN_PRICE", 5.0)
    us_paper_min_avg_dollar_volume: float = env_float("US_PAPER_MIN_AVG_DOLLAR_VOLUME", 50000000.0)
    us_paper_min_rvol: float = env_float("US_PAPER_MIN_RVOL", 1.20)
    us_paper_min_gap_pct: float = env_float("US_PAPER_MIN_GAP_PCT", 0.40)
    us_paper_max_gap_pct: float = env_float("US_PAPER_MAX_GAP_PCT", 10.0)
    us_paper_max_positions: int = env_int("US_PAPER_MAX_POSITIONS", 3)
    us_paper_max_position_pct: float = env_float("US_PAPER_MAX_POSITION_PCT", 25.0)
    us_paper_max_daily_loss_pct: float = env_float("US_PAPER_MAX_DAILY_LOSS_PCT", 2.0)
    us_paper_risk_per_trade_pct: float = env_float("US_PAPER_RISK_PER_TRADE_PCT", 0.60)
    us_paper_buy_fee_pct: float = env_float("US_PAPER_BUY_FEE_PCT", 0.01)
    us_paper_sell_fee_pct: float = env_float("US_PAPER_SELL_FEE_PCT", 0.01)
    us_paper_fx_cost_pct: float = env_float("US_PAPER_FX_COST_PCT", 0.05)
    us_paper_slippage_pct: float = env_float("US_PAPER_SLIPPAGE_PCT", 0.03)
    us_paper_max_slippage_pct: float = env_float("US_PAPER_MAX_SLIPPAGE_PCT", 0.40)
    us_paper_execution_poll_seconds: int = env_int("US_PAPER_EXECUTION_POLL_SECONDS", 5)
    us_paper_pending_expire_seconds: int = env_int("US_PAPER_PENDING_EXPIRE_SECONDS", 120)
    us_paper_stale_quote_seconds: int = env_int("US_PAPER_STALE_QUOTE_SECONDS", 120)
    # US Paper strategy tuning - V1.4 compatible controls.
    us_paper_reentry_cooldown_minutes: int = env_int("US_PAPER_REENTRY_COOLDOWN_MINUTES", 30)
    us_paper_max_entries_per_symbol_day: int = env_int("US_PAPER_MAX_ENTRIES_PER_SYMBOL_DAY", 2)
    us_paper_breakeven_r: float = env_float("US_PAPER_BREAKEVEN_R", 1.0)
    us_paper_trail_start_r: float = env_float("US_PAPER_TRAIL_START_R", 1.5)
    us_paper_weak_confirm_seconds: int = env_int("US_PAPER_WEAK_CONFIRM_SECONDS", 60)
...
class Store:
    ...
    def load_paper_state(self):
        remote=self._sb_load("paper_trading_state")
        if isinstance(remote,dict):return remote
        return self.read(DATA_DIR/"paper_trading.json",{})

    def save_paper_state(self,v):
        atomic_json_write(DATA_DIR/"paper_trading.json",v)
        if self._sb_enabled():self._sb_save("paper_trading_state",v)

    def load_us_paper_state(self):
        remote=self._sb_load("us_paper_trading_state")
        if isinstance(remote,dict):return remote
        return self.read(DATA_DIR/"us_paper_trading.json",{})

    def save_us_paper_state(self,v):
        atomic_json_write(DATA_DIR/"us_paper_trading.json",v)
        if self._sb_enabled():self._sb_save("us_paper_trading_state",v)
...
PAPER=PaperTradingEngine(
    path=DATA_DIR/"paper_trading.json",
    now_fn=now,
    buy_fee_pct=SETTINGS.namuh_buy_fee_pct,
    sell_fee_pct=SETTINGS.namuh_sell_fee_pct,
    sell_tax_pct=SETTINGS.domestic_sell_tax_pct,
    slippage_pct=SETTINGS.paper_slippage_pct,
    max_slippage_pct=SETTINGS.paper_max_slippage_pct,
    latency_seconds=SETTINGS.paper_latency_seconds,
    max_positions=SETTINGS.paper_max_positions,
    max_position_pct=SETTINGS.paper_max_position_pct,
    max_daily_loss_pct=SETTINGS.paper_max_daily_loss_pct,
    min_entry_score=SETTINGS.paper_min_entry_score,
    risk_per_trade_pct=SETTINGS.paper_risk_per_trade_pct,
    min_risk_reward=SETTINGS.paper_min_risk_reward,
    early_entry_fraction=SETTINGS.paper_early_entry_fraction,
    breakeven_r=SETTINGS.paper_breakeven_r,
    trail_start_r=SETTINGS.paper_trail_start_r,
    weak_confirm_seconds=SETTINGS.paper_weak_confirm_seconds,
    max_minute_participation_pct=SETTINGS.paper_max_minute_participation_pct,
    max_book_participation_pct=SETTINGS.paper_max_book_participation_pct,
    min_fill_ratio=SETTINGS.paper_min_fill_ratio,
    season_close_time=SETTINGS.paper_season_close_time,
    save_callback=STORE.save_paper_state,
    load_callback=STORE.load_paper_state,
)


US_PAPER=USPaperService(
    path=DATA_DIR/"us_paper_trading.json",
    now_fn=now,
    yf_module=(yf if HAS_YF and SETTINGS.us_paper_enabled else None),
    universe=[x.strip().upper() for x in SETTINGS.us_paper_universe.split(",") if x.strip()],
    candidate_limit=SETTINGS.us_paper_candidate_limit,
    scan_seconds=SETTINGS.us_paper_scan_seconds,
    focus_seconds=SETTINGS.us_paper_focus_seconds,
    min_price=SETTINGS.us_paper_min_price,
    min_avg_dollar_volume=SETTINGS.us_paper_min_avg_dollar_volume,
    min_rvol=SETTINGS.us_paper_min_rvol,
    min_gap_pct=SETTINGS.us_paper_min_gap_pct,
    max_gap_pct=SETTINGS.us_paper_max_gap_pct,
    max_positions=SETTINGS.us_paper_max_positions,
    max_position_pct=SETTINGS.us_paper_max_position_pct,
    max_daily_loss_pct=SETTINGS.us_paper_max_daily_loss_pct,
    risk_per_trade_pct=SETTINGS.us_paper_risk_per_trade_pct,
    buy_fee_pct=SETTINGS.us_paper_buy_fee_pct,
    sell_fee_pct=SETTINGS.us_paper_sell_fee_pct,
    fx_cost_pct=SETTINGS.us_paper_fx_cost_pct,
    base_slippage_pct=SETTINGS.us_paper_slippage_pct,
    max_slippage_pct=SETTINGS.us_paper_max_slippage_pct,
    execution_poll_seconds=SETTINGS.us_paper_execution_poll_seconds,
    pending_expire_seconds=SETTINGS.us_paper_pending_expire_seconds,
    stale_quote_seconds=SETTINGS.us_paper_stale_quote_seconds,
    reentry_cooldown_minutes=SETTINGS.us_paper_reentry_cooldown_minutes,
    max_entries_per_symbol_day=SETTINGS.us_paper_max_entries_per_symbol_day,
    breakeven_r=SETTINGS.us_paper_breakeven_r,
    trail_start_r=SETTINGS.us_paper_trail_start_r,
    weak_confirm_seconds=SETTINGS.us_paper_weak_confirm_seconds,
    save_callback=STORE.save_us_paper_state,
    load_callback=STORE.load_us_paper_state,
)
...
class KIS:
    ...
    def minute_bars(self,c,full_day=True):
        """KIS 당일 1분봉을 복구한다.

        FHKST03010200은 1회 호출에 최대 30건이므로 full_day=True일 때
        30분 단위 기준시각을 뒤로 이동해 정규장(09:00~15:30)을 채운다.
        중복 봉은 호출부 normalize_intraday_bars()에서 제거한다.
        """
        def fetch_at(hhmmss,retries=3):
            last_error=None
            for attempt in range(1,max(1,retries)+1):
                try:
                    r=self.s.get(
                        f"{self.rest}/uapi/domestic-stock/v1/quotations/inquire-time-itemchartprice",
                        headers={
                    ...
                    if attempt<max(1,retries):
                        wait=0.5*attempt
                        log.warning(
                            "minute bars retry %s @%s attempt=%s/%s after %.1fs: %s",
                            c,hhmmss,attempt,retries,wait,e
                        )
                        time.sleep(wait)
                    else:
                        log.warning(
                            "minute bars failed %s @%s attempts=%s: %s",
                            c,hhmmss,retries,e
                        )
            return []

        if not full_day:
            n=now();hh=min(n.hour*10000+n.minute*100+n.second,153000)
            return fetch_at(f"{hh:06d}")

        # 장중에는 미래시각 조회를 피하고, 장후/주말에는 15:30부터 복구한다.
        n=now()
        if n.weekday()<5 and (n.hour<15 or (n.hour==15 and n.minute<30)):
            if n.hour<9:
                end_minute=15*60+30  # 전 거래일 복구용; 첫 오늘 틱에서 자동 분리된다.
            else:
                end_minute=n.hour*60+n.minute
        else:
            end_minute=15*60+30
        end_minute=max(9*60,min(15*60+30,end_minute))

        rows=[];seen=set();minute=end_minute;failed_slots=[]
        while minute>=9*60:
            hh,mm=divmod(minute,60);stamp=f"{hh:02d}{mm:02d}00"
            batch=fetch_at(stamp,3)
            if not batch:
                failed_slots.append(stamp)
                # 일시적인 KIS 5xx/빈 응답 한 구간 때문에 과거 전체 복구를 중단하지 않는다.
                # 해당 30분 구간만 건너뛰고 계속 이전 시간대를 조회한다.
                minute-=30
                time.sleep(0.35)
                continue
            for r in batch:
                key=(str(r.get("stck_cntg_hour") or ""),str(r.get("stck_prpr") or ""),str(r.get("acml_vol") or ""))
                if key not in seen:
        ...
        if failed_slots:
            log.warning(
                "minute history partial gaps: %s failed_slots=%s restored=%s",
                STATE.vip_targets.get(c,c),",".join(failed_slots),len(rows)
            )
        log.info("minute history restored: %s %s rows",STATE.vip_targets.get(c,c),len(rows))
        return rows
    ...
    def trade_codes(self):
        # VIP에서 제거돼도 Paper Trader 보유/대기 주문은 청산 완료까지 계속 실시간 체결 구독.
        return list(dict.fromkeys(list(STATE.vip_targets)+list(PAPER.tracked_codes())))
    def order_codes(self):
        if not SETTINGS.enable_orderbook:return []
        # 호가 슬롯 수는 늘리지 않는다. 가상 보유종목을 우선 배치하고 남는 슬롯만 VIP 집중호가에 사용.
        held=[c for c in PAPER.tracked_codes() if PAPER.has_position(c)]
        return list(dict.fromkeys(held+list(STATE.focus_codes)))[:SETTINGS.focus_orderbook_slots]
...
class App:
    ...
    def sync_bars_for(self,c):
        # 신규 VIP 추가 직후에는 daily_trend가 아직 없을 수 있으므로
        # 분봉에 날짜를 붙이기 전에 최근 거래일(asof)을 먼저 확보한다.
        if not str((STATE.daily_trend.get(c) or {}).get("asof") or ""):
            try:self.sync_daily_for(c,True)
            except Exception as e:log.warning("minute restore daily-asof prep %s: %s",c,e)
        raw=KIS_CLIENT.minute_bars(c,full_day=True);rows=[];n=now();current_minute=n.replace(second=0,microsecond=0)
        # 주말/장후 재배포에서도 마지막 거래일 분봉을 버리지 않도록 일봉 기준일을 사용한다.
        asof=str((STATE.daily_trend.get(c) or {}).get("asof") or "")
        try:
            hist_date=datetime.strptime(asof,"%Y%m%d").date() if len(asof)==8 else n.date()
        except Exception:
            hist_date=n.date()
        # 평일 정규장 진행 중에는 REST 분봉을 오늘 날짜로 간주한다.
        if n.weekday()<5 and (n.hour>9 or (n.hour==9 and n.minute>=0)) and (n.hour<15 or (n.hour==15 and n.minute<=35)):
            hist_date=n.date()
        for r in raw:
            try:
                h=str(r.get("stck_cntg_hour") or "").zfill(6)
                m=datetime(hist_date.year,hist_date.month,hist_date.day,int(h[:2]),int(h[2:4]),0,tzinfo=KST)
                # 오늘 진행 중 현재분만 NXT current와 겹칠 수 있어 제외한다.
                if hist_date==n.date() and m>=current_minute:continue
        ...
        rows=normalize_intraday_bars(rows,hist_date)
        if rows:
            with STATE.lock:
                STATE.bars[c]=deque(rows,maxlen=SETTINGS.intraday_max_bars)
            log.info("intraday restore ready: %s date=%s bars=%s",STATE.vip_targets.get(c,c),hist_date,len(rows))
    ...
    def _nextday_intraday_rows(self,c):
        ...
        # now().date()가 아니라 보유한 가장 최근 거래일을 기준으로 한다.
        # 주말/휴일/장후 Render 재시작 후에도 금요일(최근 거래일) 데이터가 사라지지 않는다.
        latest_date=max((kst_dt(b.minute).date() for b in rows),default=None)
        bars=normalize_intraday_bars(rows,latest_date) if latest_date else []
        out=[]
        for b in bars:
    ...
    def nextday_analysis(self,c,force=False):
        ...
        try:
            ...
            # 엔진의 실제 최소조건(일봉/30분봉)과 동일한 기준으로 선검증한다.
            # 기존의 단순 "1분봉 60개" 기준은 30분봉 8개를 보장하지 못해 422/500이 엇갈릴 수 있었다.
            buckets=set()
            for row in intraday_rows:
                try:
                    dt=datetime.fromisoformat(str(row.get("time") or ""))
                    buckets.add(dt.replace(minute=(dt.minute//30)*30,second=0,microsecond=0))
                except Exception:
                    pass
            if len(daily_rows)<SETTINGS.nextday_min_daily_bars or len(buckets)<SETTINGS.nextday_min_30m_bars:
                return {
                    "ok":False,
                    "error":"data_not_ready",
                    "message":(
                        f"내일흐름 분석 데이터가 아직 부족합니다. "
                        f"일봉 {len(daily_rows)}/{SETTINGS.nextday_min_daily_bars}, "
                        f"정규장 30분구간 {len(buckets)}/{SETTINGS.nextday_min_30m_bars} "
                        f"(1분봉 {len(intraday_rows)}개). Render 재시작 직후라면 잠시 후 다시 시도해주세요."
                    ),
                    "daily_bars":len(daily_rows),
                    "intraday_bars":len(intraday_rows),
                    "bars_30m":len(buckets),
                }
    ...
    def paper_watch_name(self,c):
        if c in STATE.vip_targets:return STATE.vip_targets[c]
        p=PAPER.state.get("positions",{}).get(c) or {}
        o=PAPER.state.get("pending_orders",{}).get(c) or {}
        return str(p.get("name") or o.get("name") or MASTER.code_to_name.get(c) or c)

    def ensure_paper_watch_target(self,c):
        if c not in STATE.targets:
            STATE.targets[c]=TargetState(c,self.paper_watch_name(c),stage_changed_at=now().isoformat())
        if c not in STATE.daily_trend:WORKER.submit(self.sync_daily_for,c)
        if c not in STATE.bars and c not in STATE.current:WORKER.submit(self.sync_bars_for,c)

    def cleanup_detached_paper_target(self,c):
        if c in STATE.vip_targets or c in POSITIONS.data or c in PAPER.tracked_codes():return
        with STATE.lock:
            STATE.targets.pop(c,None);STATE.bars.pop(c,None);STATE.current.pop(c,None)
            STATE.last_cum.pop(c,None);STATE.books.pop(c,None);STATE.last_tick_at.pop(c,None)
            STATE.daily_trend.pop(c,None);STATE.daily_trend_at.pop(c,None)
        self.update_focus()
    ...
    def on_tick(self,t):
        ...
        with STATE.lock:
            ...
            if not cur:
                ...
                STATE.bars[t.code]=deque(normalize_intraday_bars(completed,minute.date()),maxlen=SETTINGS.intraday_max_bars)
                STATE.current[t.code]=Bar(minute,t.price,t.price,t.price,t.price,inc,t.cumulative_volume,t.trade_strength)
        ...
        is_vip=t.code in STATE.vip_targets
        is_paper=t.code in PAPER.tracked_codes()
        if is_vip or is_paper:
            if is_paper and not is_vip:self.ensure_paper_watch_target(t.code)
            target=STATE.targets.get(t.code)
            if target:
                target.last_price=t.price;target.trade_strength=t.trade_strength
            if t.code not in STATE.daily_trend:WORKER.submit(self.sync_daily_for,t.code)
            if is_vip:self.evaluate_target(t.code)
            try:
                pm=BRAIN.metrics(t.code) or {}
                pob=STATE.books.get(t.code);pcur=STATE.current.get(t.code)
                if pob:
                    pm=dict(pm);pm.update({
                        "total_ask":pob.total_ask,"total_bid":pob.total_bid,
                        "best_ask":pob.best_ask,"best_bid":pob.best_bid,"imbalance":pob.imbalance,
                    })
                if pcur:
                    pm=dict(pm);pm["current_minute_volume"]=pcur.volume
                watch_codes=set(STATE.vip_targets)|PAPER.tracked_codes()
                all_prices={}
                for c in watch_codes:
                    tt=STATE.targets.get(c)
                    if tt and tt.last_price:all_prices[c]=tt.last_price
                    elif STATE.current.get(c):all_prices[c]=STATE.current[c].close
                stage=(STATE.targets.get(t.code).stage if STATE.targets.get(t.code) else ST_WAIT)
                PAPER.on_market(
                    code=t.code,name=self.paper_watch_name(t.code),price=t.price,
                    metrics=pm,stage=stage,market_risk=STATE.market_risk,
                    all_prices=all_prices,metrics_by_code={t.code:pm},
                    allow_entry=is_vip,
                )
            except Exception as e:log.warning("paper trader tick %s: %s",t.code,e)
            if not is_vip:self.cleanup_detached_paper_target(t.code)
    ...
    def add_vip(self,q,chat):
        ...
        STATE.vip_targets[c]=n;STATE.ensure_target(c,n);STATE.save_vip();WORKER.submit(self.sync_daily_for,c,True);WORKER.submit(self.sync_bars_for,c);BOT.send(f"🎯 {html.escape(n)} VIP 추가 완료",chat,buttons=self.target_buttons(c))
    def remove_vip(self,c,chat):
        if c in POSITIONS.data:BOT.send("수동 보유관리 중이라 삭제 불가",chat);return
        paper_hold=PAPER.has_position(c)
        PAPER.cancel_pending_buy(c,"VIP_REMOVED")
        n=STATE.vip_targets.pop(c,None)
        continued=bool(paper_hold or PAPER.has_pending(c))
        if not continued:STATE.targets.pop(c,None)
        STATE.save_vip();self.update_focus()
        if continued:
            BOT.send(f"🗑️ {html.escape(n or c)} VIP 삭제 완료\n💼 가상보유/매도대기 포지션은 청산 완료까지 NXT 실시간 감시를 계속합니다.\n신규 가상매수는 즉시 차단됩니다.",chat,buttons=self.main_buttons())
        else:
            BOT.send(f"🗑️ {html.escape(n or c)} 삭제 완료",chat,buttons=self.main_buttons())
    ...
    def start(self):
        ...
        # 재배포 시에도 VIP에서 빠진 Paper 보유종목은 청산 완료까지 복구한다.
        for c in list(PAPER.tracked_codes()):
            if c not in STATE.vip_targets:
                PAPER.cancel_pending_buy(c,"STARTUP_NOT_VIP")
            if PAPER.has_position(c) or PAPER.has_pending(c):
                self.ensure_paper_watch_target(c)

        # 재배포 직후 신호보다 데이터 복구가 먼저다.
        STATE.runtime["boot_phase"]="restoring_market_data"
        restore_codes=list(dict.fromkeys(list(STATE.vip_targets)+list(PAPER.tracked_codes())))
        for c in restore_codes:
            # 최근 거래일(asof)을 먼저 확보한 뒤 분봉을 복구해야 주말/휴일 날짜가 틀어지지 않는다.
            try:self.sync_daily_for(c,True)
        ...
        if SETTINGS.us_paper_enabled:
            threading.Thread(target=US_PAPER.run,daemon=True,name="US-paper-worker").start()
...
def health():
    us=US_PAPER.summary()
    return jsonify({"status":"ok","version":SETTINGS.version,"vip":len(STATE.vip_targets),"positions":len(POSITIONS.data),"market_risk":STATE.market_risk,"runtime":STATE.runtime,"us_paper":{"enabled":us.get("enabled"),"active":us.get("active"),"market_status":us.get("market_status")}})
...
def miniapp_state():
    ...
    return _no_store(make_response(jsonify({"ok":True,"version":SETTINGS.version,"server_time":now().isoformat(),
        ...
        "paper":_paper_summary(),"us_paper":US_PAPER.summary()})))
...
def miniapp_vip_add():
    ...
    STATE.vip_targets[c]=n;STATE.ensure_target(c,n);STATE.save_vip();WORKER.submit(APP.sync_daily_for,c,True);WORKER.submit(APP.sync_bars_for,c);APP.update_focus()
...
def miniapp_vip_remove(code):
    ...
    code,t=_mini_target(code)
    if not t:return jsonify({"ok":False,"error":"not_found"}),404
    if code in POSITIONS.data:return jsonify({"ok":False,"error":"position_active","message":"수동 보유관리 중이라 삭제할 수 없습니다."}),409
    paper_hold=PAPER.has_position(code)
    PAPER.cancel_pending_buy(code,"VIP_REMOVED")
    n=STATE.vip_targets.pop(code,None)
    continued=bool(paper_hold or PAPER.has_pending(code))
    if not continued:STATE.targets.pop(code,None)
    STATE.save_vip();APP.update_focus()
    msg="VIP 삭제 완료"
    if continued:msg+=" · 가상보유/매도대기 종목은 청산 완료까지 백그라운드 실시간 감시"
    return jsonify({"ok":True,"code":code,"name":n,"paper_tracking":continued,"message":msg})
...
def _paper_watch_codes():
    return set(STATE.vip_targets)|PAPER.tracked_codes()

def _paper_prices():
    rows={}
    for c in _paper_watch_codes():
        t=STATE.targets.get(c)
        if t and t.last_price:rows[c]=t.last_price
        elif STATE.current.get(c):rows[c]=STATE.current[c].close
    return rows

def _paper_metrics():
    rows={}
    for c in _paper_watch_codes():
        m=BRAIN.metrics(c) or {}
        ob=STATE.books.get(c);cur=STATE.current.get(c)
        if ob:
            m=dict(m);m.update({"total_ask":ob.total_ask,"total_bid":ob.total_bid,"imbalance":ob.imbalance})
        if cur:
            m=dict(m);m["current_minute_volume"]=cur.volume
        rows[c]=m
    return rows

def _paper_summary():
    s=PAPER.summary(_paper_prices(),_paper_metrics())
    for x in s.get("positions",[]):
        x["vip_active"]=x.get("code") in STATE.vip_targets
    for x in s.get("pending_orders",[]):
        x["vip_active"]=x.get("code") in STATE.vip_targets
    s["detached_tracking_count"]=sum(1 for x in s.get("positions",[]) if not x.get("vip_active"))
    return s

@web.get("/api/miniapp/paper")
def miniapp_paper():
    if not _mini_auth():return jsonify({"ok":False,"error":"telegram_auth_required"}),401
    return _no_store(make_response(jsonify({"ok":True,"paper":_paper_summary(),"server_time":now().isoformat()})))

@web.post("/api/miniapp/paper/start")
def miniapp_paper_start():
    if not _mini_auth():return jsonify({"ok":False,"error":"telegram_auth_required"}),401
    d=_mini_json()
    try:return jsonify({"ok":True,"message":"가상투자 새 시즌 시작","paper":PAPER.start_new_season(num(d.get("capital")),integer(d.get("days"),5))})
    except RuntimeError as e:return jsonify({"ok":False,"error":"positions_active","message":str(e)}),409
    except Exception as e:return jsonify({"ok":False,"error":"invalid_setting","message":str(e)}),400

@web.post("/api/miniapp/paper/liquidate")
def miniapp_paper_liquidate():
    if not _mini_auth():return jsonify({"ok":False,"error":"telegram_auth_required"}),401
    rows=PAPER.force_liquidate(_paper_prices(),_paper_metrics())
    for c in list(STATE.targets):APP.cleanup_detached_paper_target(c)
    return jsonify({"ok":True,"message":f"가상 보유종목 {len(rows)}개 전량매도 처리","trades":rows,"paper":_paper_summary()})

@web.post("/api/miniapp/paper/pause")
def miniapp_paper_pause():
    if not _mini_auth():return jsonify({"ok":False,"error":"telegram_auth_required"}),401
    PAPER.pause();return jsonify({"ok":True,"message":"가상투자 일시정지","paper":_paper_summary()})

@web.post("/api/miniapp/paper/resume")
def miniapp_paper_resume():
    if not _mini_auth():return jsonify({"ok":False,"error":"telegram_auth_required"}),401
    PAPER.resume();return jsonify({"ok":True,"message":"가상투자 재개","paper":_paper_summary()})

@web.get("/api/miniapp/us-paper")
def miniapp_us_paper():
    if not _mini_auth():return jsonify({"ok":False,"error":"telegram_auth_required"}),401
    return _no_store(make_response(jsonify({"ok":True,"paper":US_PAPER.summary(),"server_time":now().isoformat()})))

@web.post("/api/miniapp/us-paper/start")
def miniapp_us_paper_start():
    if not _mini_auth():return jsonify({"ok":False,"error":"telegram_auth_required"}),401
    if not SETTINGS.us_paper_enabled or not HAS_YF:
        return jsonify({"ok":False,"error":"us_market_data_unavailable","message":"미국장 데이터 모듈(yfinance)이 비활성 상태입니다."}),503
    d=_mini_json()
    try:
        return jsonify({"ok":True,"message":"🇺🇸 미국장 가상투자 새 시즌 시작","paper":US_PAPER.start_new_season(num(d.get("capital")),integer(d.get("days"),5))})
    except RuntimeError as e:return jsonify({"ok":False,"error":"positions_active","message":str(e)}),409
    except Exception as e:return jsonify({"ok":False,"error":"invalid_setting","message":str(e)}),400

@web.post("/api/miniapp/us-paper/liquidate")
def miniapp_us_paper_liquidate():
    if not _mini_auth():return jsonify({"ok":False,"error":"telegram_auth_required"}),401
    rows=US_PAPER.force_liquidate()
    return jsonify({"ok":True,"message":f"🇺🇸 미국 가상보유 {len(rows)}개 전량매도 처리","trades":rows,"paper":US_PAPER.summary()})

@web.post("/api/miniapp/us-paper/pause")
def miniapp_us_paper_pause():
    if not _mini_auth():return jsonify({"ok":False,"error":"telegram_auth_required"}),401
    US_PAPER.pause();return jsonify({"ok":True,"message":"🇺🇸 미국장 가상투자 일시정지","paper":US_PAPER.summary()})

@web.post("/api/miniapp/us-paper/resume")
def miniapp_us_paper_resume():
    if not _mini_auth():return jsonify({"ok":False,"error":"telegram_auth_required"}),401
    US_PAPER.resume();return jsonify({"ok":True,"message":"🇺🇸 미국장 가상투자 재개","paper":US_PAPER.summary()})

@web.post("/api/miniapp/us-paper/scan")
def miniapp_us_paper_scan():
    if not _mini_auth():return jsonify({"ok":False,"error":"telegram_auth_required"}),401
    if not SETTINGS.us_paper_enabled or not HAS_YF:
        return jsonify({"ok":False,"error":"us_market_data_unavailable"}),503
    WORKER.submit(US_PAPER.scan_universe)
    return jsonify({"ok":True,"message":"🇺🇸 미국장 저부하 후보 스캔을 요청했습니다."})
