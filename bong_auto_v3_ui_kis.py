...
class Settings:
    ...
    listing_cache:str=os.getenv('LISTING_CACHE','bong_listing_cache.json')
SETTINGS=Settings()
...
# ===== infrastructure.py =====
import json,logging,threading,time,io,zipfile
...
class Telegram:
    def __init__(self):
        self.s=requests.Session(); self.offset=0; self.handler:Optional[Callable[[str,str],None]]=None
    def _api(self,method,payload):
        if not SETTINGS.telegram_token:return None
        try:
            r=self.s.post(f'https://api.telegram.org/bot{SETTINGS.telegram_token}/{method}',json=payload,timeout=15); r.raise_for_status(); return r.json()
        except Exception as e: log.warning('Telegram %s: %s',method,e); return None
    def send(self,text,chat=None,buttons=None):
        ...
        d=self._api('sendMessage',payload); return bool(d and d.get('ok'))
    def edit(self,chat,message_id,text,buttons=None):
        payload={'chat_id':str(chat),'message_id':message_id,'text':text,'parse_mode':'HTML','disable_web_page_preview':True}
        if buttons: payload['reply_markup']={'inline_keyboard':buttons}
        d=self._api('editMessageText',payload); return bool(d and d.get('ok'))
    ...
    def poll(self):
        ...
        while True:
            try:
                r=self.s.get(f'https://api.telegram.org/bot{SETTINGS.telegram_token}/getUpdates',params={'timeout':25,'offset':self.offset,'allowed_updates':json.dumps(['message','callback_query'])},timeout=35)
                ...
                for u in r.json().get('result',[]):
                    ...
                    if cq:
                        self.answer_callback(str(cq.get('id') or ''))
                        m=cq.get('message') or {}; chat=str((m.get('chat') or {}).get('id') or ''); data=str(cq.get('data') or '').strip()
                        if data and chat and self.handler: threading.Thread(target=self.handler,args=(data,chat),daemon=True).start()
                        continue
...
class KIS:
    ...
    def stream(self,codes_fn,names,on_tick,on_book,state):
        ...
        try:
            retry=5
            duplicate_wait=60
            adaptive_trade=max(1,SETTINGS.ws_trade_limit)
            adaptive_order=max(0,min(SETTINGS.ws_orderbook_limit,adaptive_trade))
            while True:
                ...
                try:
                    ...
                    def desired_codes():
                        ...
                        trade_codes=all_codes[:max(1,adaptive_trade)]
                        order_codes=trade_codes[:max(0,min(adaptive_order,len(trade_codes)))]
                    ...
                    refresh_stop.set()
                    for worker in (initial_thread,refresh_thread):
                    ...
                    if duplicate_appkey:
                        ...
                        log.warning('NXT 구독 한도 초과 → 체결 %s / 호가 %s로 하향 후 120초 대기',adaptive_trade,adaptive_order)
                        time.sleep(120)
                        retry=5
                    else:
                        time.sleep(retry)
...
class State:
    ...
    def _save_listing_cache(self,names,meta):
        try:
            payload={'saved_at':now().isoformat(),'names':names,'meta':meta}
            Path(SETTINGS.listing_cache).write_text(
                json.dumps(payload,ensure_ascii=False,default=str),
                encoding='utf-8'
            )
        except Exception as e:
            log.warning('종목 목록 캐시 저장 실패: %s',e)

    def _load_listing_cache(self):
        try:
            p=Path(SETTINGS.listing_cache)
            if not p.exists():
                return None,None
            ...
            meta=d.get('meta') or {}
            if names and meta:
                log.warning('KIS 마스터 대신 마지막 정상 캐시 %s개를 사용합니다.',len(names))
                return names,meta
        except Exception as e:
            log.warning('종목 목록 캐시 복원 실패: %s',e)
        return None,None

    @staticmethod
    def _split_fixed_width(raw,widths):
        out=[]; pos=0
        for w in widths:
            out.append(raw[pos:pos+w].strip())
            pos+=w
        return out

    def _download_kis_master(self,market):
        """한국투자 공식 국내주식 종목 마스터파일을 내려받아 최소 메타데이터로 변환한다."""
        if market=='KOSPI':
            ...
            idx={'industry':2,'etp':12,'spac':19,'base':31,'halt':34,'manage':36,'volume':47,'marcap':65}
        else:
            ...
            idx={'industry':2,'etp':8,'spac':14,'base':26,'halt':29,'manage':31,'volume':42,'marcap':59}

        r=requests.get(url,timeout=30,headers={'User-Agent':'Mozilla/5.0'})
        r.raise_for_status()
        with zipfile.ZipFile(io.BytesIO(r.content)) as zf:
            target=mst_name if mst_name in zf.namelist() else next((x for x in zf.namelist() if x.endswith('.mst')),None)
            if not target:
                raise RuntimeError(f'{market} 마스터 .mst 파일 없음')
            raw=zf.read(target).decode('cp949',errors='ignore')

        names={}; meta={}
        for line in raw.splitlines():
            if len(line)<=tail_size:
                continue
            head=line[:-tail_size]
            tail=line[-tail_size:]
            short=head[0:9].strip()
            name=head[21:].strip()
            ...
            vals=self._split_fixed_width(tail,widths)
            base=integer(vals[idx['base']]) if idx['base']<len(vals) else 0
            vol=integer(vals[idx['volume']]) if idx['volume']<len(vals) else 0
            marcap_eok=num(vals[idx['marcap']]) if idx['marcap']<len(vals) else 0
            marcap=marcap_eok*100_000_000 if marcap_eok>0 else 0
            amount=base*vol if base>0 and vol>0 else 0
            industry=vals[idx['industry']] if idx['industry']<len(vals) else ''
            etp=vals[idx['etp']].strip().upper() if idx['etp']<len(vals) else ''
            spac=vals[idx['spac']].strip().upper() if idx['spac']<len(vals) else ''
            halt=vals[idx['halt']].strip().upper() if idx['halt']<len(vals) else ''
            manage=vals[idx['manage']].strip().upper() if idx['manage']<len(vals) else ''

            names[code]=name
            meta[code]={
                'Code':code,'Name':name,'Market':market,
                'Close':base,'Volume':vol,'Marcap':marcap,'Amount':amount,
                'ChangesRatio':0.0,
                'Sector':f'{market}-{industry or "기타"}',
                'Industry':f'{market}-{industry or "기타"}',
                'ETP':etp,'SPAC':spac,'TradingHalt':halt,'Management':manage,
            }

        if not names:
            raise RuntimeError(f'{market} KIS 마스터 파싱 결과가 비어 있습니다.')
        return names,meta

    def load(self):
        """KRX/FDR StockListing 대신 KIS 공식 마스터파일로 종목목록을 구성한다."""
        names={}; meta={}; errors=[]
        for market in ('KOSPI','KOSDAQ'):
            loaded=False
            for attempt in range(1,4):
                try:
                    n,m=self._download_kis_master(market)
                    names.update(n); meta.update(m)
                    log.info('KIS %s 마스터 로드 성공: %s개',market,len(n))
                    loaded=True
                    break
                except Exception as e:
                    errors.append(f'{market} {attempt}/3: {e}')
                    log.warning('KIS %s 마스터 로드 실패 %s/3: %s',market,attempt,e)
                    time.sleep(min(2*attempt,6))
            if not loaded:
                log.error('KIS %s 마스터 최종 실패',market)

        if names and meta:
            with self.lock:
                self.names=names
                self.meta=meta
                self.runtime['last_error']=None if len(names)>1000 else f'종목목록 부분 로드: {len(names)}개'
            self._save_listing_cache(names,meta)
            return

        names,meta=self._load_listing_cache()
        if names and meta:
            with self.lock:
                self.names=names
                self.meta=meta
                self.runtime['last_error']='KIS 마스터 실시간 로드 실패 → 캐시 사용'
            return

        raise RuntimeError('KIS 종목 마스터와 캐시 복원 모두 실패: '+' | '.join(errors[-6:]))
    ...
    def refresh(self):
        ...
        with self.lock:
            for c,m in self.meta.items():
                ...
                if str(m.get('TradingHalt') or '').upper() in ('Y','1'):continue
                if str(m.get('Management') or '').upper() in ('Y','1'):continue
                if str(m.get('SPAC') or '').upper() in ('Y','1'):continue
                if str(m.get('ETP') or '').upper() in ('Y','1'):continue
    ...
    def push(self,t:Tick):
        with self.lock:
            if t.price<=0 or t.volume<0 or t.cumulative_volume<0:
                self.runtime['last_error']=f'비정상 실시간 데이터 차단 {t.code}'
                log.warning('%s price=%s vol=%s cum=%s',self.runtime['last_error'],t.price,t.volume,t.cumulative_volume)
                return None
            key=(t.market,t.code)
            minute=t.timestamp.replace(second=0,microsecond=0)
            previous=self.last_cum.get(key)
            if previous is not None and t.cumulative_volume>0 and previous>0 and t.cumulative_volume<previous:
                log.warning('누적거래량 역행 감지 %s: %s -> %s',t.code,previous,t.cumulative_volume)
                inc=0
            else:
                last=previous if previous is not None else t.cumulative_volume
...
class App:
 ...
 def on_tick(self,t):
  ...
  if age>120 or age<-30:
   STATE.runtime['last_error']=f'비정상 시각 틱 차단 {t.code}: {age:.0f}초'
   log.warning(STATE.runtime['last_error'])
   return
  b=STATE.push(t)
  if t.price<=0:return
 ...
 def recommend(self,c):
  ...
  if STATE.runtime.get('ws_nxt')!='connected' or not STATE.runtime.get('last_tick'):
   log.warning('추천 차단: NXT 연결 또는 최근 체결 데이터 없음')
   return
  try:
   last_dt=datetime.fromisoformat(STATE.runtime['last_tick'])
   if (now()-last_dt).total_seconds()>120:
    STATE.runtime['last_error']='추천 차단: NXT 체결 120초 이상 지연'
    log.warning(STATE.runtime['last_error'])
    return
  except Exception as e:
   STATE.runtime['last_error']=f'추천 차단: 마지막 체결시각 오류 {e}'
   return
  if len(self.recs)>=SETTINGS.daily_limit or any(x['code']==c.code and x['kind']==c.kind for x in self.recs):return
  self.recs.append({'code':c.code,'kind':c.kind,'time':now(),'price':c.price});self.last_rec=now();STATE.active[c.code]=c.kind;BOT.send(self.score_text(c),buttons=self.detail_buttons(c.code));DATABASE.insert('recommendations',{'stock_code':c.code,'stock_name':c.name,'recommended_price':c.price,'confidence_score':c.reliability,'recommendation_time':now().isoformat()})
 ...
 def main_buttons(self):
  return [
   [{'text':'📈 시장현황','callback_data':'/시장'},{'text':'🎯 오늘추천','callback_data':'/오늘추천'}],
   [{'text':'⚡ 단타','callback_data':'/단타'},{'text':'📈 스윙','callback_data':'/스윙'},{'text':'🌙 종가배팅','callback_data':'/종배'}],
   [{'text':'💼 보유종목','callback_data':'/보유'},{'text':'⭐ 관심종목','callback_data':'/관심목록'}],
   [{'text':'📊 추천성적','callback_data':'/성과'},{'text':'🩺 봇진단','callback_data':'/상태'}],
   [{'text':'🌅 아침비교','callback_data':'/아침비교'},{'text':'🌙 야간브리핑','callback_data':'/야간브리핑'}]
  ]
 def back_buttons(self):
  return [[{'text':'🏠 메인','callback_data':'/메뉴'}]]
 def menu_text(self):
  return f'🤖 <b>뽕실봇 AUTO CONTROL</b>\n\n현재 단계: <b>{self.auto_phase()}</b>\nNXT: <b>{STATE.runtime.get("ws_nxt")}</b>\n오늘 추천: <b>{len(self.recs)}/{SETTINGS.daily_limit}</b>\n\n원하는 기능을 선택하세요.'
 def holding_buttons(self):
  rows=[]
  for c,x in list(POSITION.data.items())[:8]: rows.append([{'text':f'💼 {x.name}','callback_data':f'/보유상세 {c}'}])
  rows.append([{'text':'🔄 새로고침','callback_data':'/보유'},{'text':'🏠 메인','callback_data':'/메뉴'}]); return rows
 def watch_buttons(self):
  rows=[]
  for c,n in list(STATE.watch.items())[:8]: rows.append([{'text':f'⭐ {n}','callback_data':f'/분석 {c}'}])
  rows.append([{'text':'🏠 메인','callback_data':'/메뉴'}]); return rows
 def detail_text(self,c):
  day=BRAIN.evaluate_day(c); swing=BRAIN.evaluate_swing(c)
  try: close=BRAIN.evaluate_close(c)
  except Exception: close=None
  price=day.price or swing.price or (close.price if close else 0)
  return (f'🔎 <b>{html.escape(STATE.names.get(c,c))}</b> ({c})\n\n현재가 <b>{price:,.0f}원</b>\n단타 {day.score:.1f}/100 · {day.stage}\n스윙 {swing.score:.1f}/100 · {swing.stage}\n종배 {(close.score if close else 0):.1f}/100 · {(close.stage if close else "확인불가")}\n\n※ 버튼은 조회·감시용이며 자동주문은 하지 않습니다.')
 def detail_buttons(self,c):
  watch_text='⭐ 관심삭제' if c in STATE.watch else '⭐ 관심등록'
  watch_cmd=f'/관심삭제 {c}' if c in STATE.watch else f'/관심등록 {c}'
  return [[{'text':'⚡ 단타분석','callback_data':f'/분석단타 {c}'},{'text':'📈 스윙분석','callback_data':f'/분석스윙 {c}'}],[{'text':'🌙 종배분석','callback_data':f'/분석종배 {c}'},{'text':watch_text,'callback_data':watch_cmd}],[{'text':'📚 호가','callback_data':f'/호가 {c}'},{'text':'💼 매수등록 안내','callback_data':f'/매수안내 {c}'}],[{'text':'🏠 메인','callback_data':'/메뉴'}]]
 def handle(self,text,chat):
  p=text.split();cmd=p[0]
  if cmd in ('/start','/메뉴','/menu'):
   BOT.send(self.menu_text(),chat,self.main_buttons());return
  if cmd=='/오늘추천':
   today=now().date(); rows=[x for x in self.recs if x['time'].date()==today]
   if not rows: BOT.send('📭 오늘 발생한 확정 추천 시그널이 없습니다.',chat,self.back_buttons())
   else:
    lines=['🎯 <b>오늘 추천</b>','']+[f"{i}. {html.escape(STATE.names.get(x['code'],x['code']))} ({x['code']}) · {x['kind']} · {x['price']:,.0f}원" for i,x in enumerate(rows,1)]
    buttons=[[{'text':f"🔎 {STATE.names.get(x['code'],x['code'])}",'callback_data':f"/분석 {x['code']}"}] for x in rows[:8]]+self.back_buttons()
    BOT.send('\n'.join(lines),chat,buttons)
   return
  if cmd=='/분석' and len(p)>1:
   c=self.resolve(' '.join(p[1:])); BOT.send(self.detail_text(c),chat,self.detail_buttons(c)) if c else BOT.send('종목명을 정확히 입력해 주세요.',chat); return
  if cmd in ('/분석단타','/분석스윙','/분석종배') and len(p)>1:
   c=self.resolve(p[1]); card=(BRAIN.evaluate_day(c) if cmd=='/분석단타' else BRAIN.evaluate_swing(c) if cmd=='/분석스윙' else BRAIN.evaluate_close(c)) if c else None
   BOT.send(self.score_text(card),chat,self.detail_buttons(c)) if card else BOT.send('분석할 수 없습니다.',chat); return
  if cmd=='/매수안내' and len(p)>1:
   c=self.resolve(p[1]); n=STATE.names.get(c,c) if c else p[1]
   BOT.send(f'💼 <b>{html.escape(n)} 매수감시 등록</b>\n\n자동주문은 하지 않습니다. 실제 체결 후 아래 형식으로 입력하세요.\n<code>/매수 {html.escape(n)} 매수가 수량 단타</code>\n또는 마지막 유형을 스윙/종배로 바꿀 수 있습니다.',chat,self.back_buttons()); return
  if cmd=='/보유상세' and len(p)>1:
   c=self.resolve(p[1]); x=POSITION.data.get(c) if c else None
   if not x: BOT.send('보유종목을 찾을 수 없습니다.',chat,self.back_buttons()); return
   tick=STATE.ticks.get(('NXT',c)); cur=tick.price if tick else 0
   if cur<=0:
    try: cur=num(KIS_CLIENT.price(c).get('stck_prpr'))
    except Exception: cur=0
   gain=pct(cur,x.entry) if cur else 0
   BOT.send(f'💼 <b>{html.escape(x.name)}</b> ({c})\n매수가 {x.entry:,.0f}원 · 현재가 {cur:,.0f}원\n수익률 <b>{gain:+.2f}%</b>\n방어선 {x.stop:,.0f}원\n1차 {x.target1:,.0f}원 · 2차 {x.target2:,.0f}원 · 최종 {x.target3:,.0f}원\n상태 {x.state}',chat,[[{'text':'📚 호가','callback_data':f'/호가 {c}'},{'text':'🔎 종목분석','callback_data':f'/분석 {c}'}],[{'text':'◀️ 보유목록','callback_data':'/보유'},{'text':'🏠 메인','callback_data':'/메뉴'}]]); return
  if cmd in ('/도움말','/help'):BOT.send('<b>명령어</b>\n/상태 /단타 /스윙 /종배 /종배후보 /종배결과 /예비후보 /시장 /성과\n/매수 종목 매수가 수량 [단타|스윙|종배]\n/매도 종목 /보유 /보유리셋\n/관심등록 종목 /관심삭제 종목 /관심목록\n/호가 종목 /후보갱신\n/야간브리핑 /내일추천 /아침비교',chat)
   ...
   BOT.send(self.holding_text(),chat,self.holding_buttons())
  ...
  elif cmd=='/관심목록':BOT.send('👀 <b>관심목록</b>\n'+('\n'.join(f'• {n} ({c})' for c,n in STATE.watch.items()) or '등록된 관심종목이 없습니다.'),chat,self.watch_buttons())
  ...
  else:BOT.send('명령어는 /도움말 또는 아래 메뉴를 이용하세요.',chat,self.main_buttons())
 def scheduler(self):
  sent={}
  while True:
