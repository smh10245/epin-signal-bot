...
class Settings:
    version: str = "5.0.4 VIP Repeat Sniper Stable"
...
def us_market_summary():
    """FRED 일별 종가로 미국 3대 지수의 직전 거래일 등락률을 계산한다.
    외부 조회 실패 시 마지막 성공 캐시를 사용한다.
    """
    cache=STORE.load_us_market() if hasattr(STORE,"load_us_market") else {}
    try:
        # FRED CSV는 API 키 없이 일별 종가를 받을 수 있고 Yahoo Rate Limit과 분리된다.
        ...
        for label,series_id in series:
            ...
            rows.append(f"{label} {pct(last,prev):+.2f}%")
            observed_dates.append(last_date)

        text=" · ".join(rows)
        data_date=min(observed_dates) if observed_dates else "-"
        payload={
            "text":text,
        ...
        log.warning("US market FRED fetch failed: %s",e)
        if cache.get("text"):
            return (
                f"{cache['text']}\n"
