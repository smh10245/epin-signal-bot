...
class Settings:
    version: str = "5.1.0 VIP Repeat Sniper + Mini App"
...
class Telegram:
    ...
    def configure_miniapp_menu(self):
        ...
        d=self._req("setChatMenuButton",payload,12)
...
class Brain:
    @staticmethod
    def bootstrap_metrics(c):
        """거래량이 0이어도 최근 분봉 가격만으로 장전용 지지/저항/VWAP 기준값을 만든다."""
        ...
        r=bars[-40:]
        latest=r[-1]
        highs=[b.high for b in r if b.high>0]
        lows=[b.low for b in r if b.low>0]
        closes=[b.close for b in r if b.close>0]
        ...
        if tot>0:
            ...
            # 과거 분봉의 체결량이 비어 있어도 장전 브리핑이 0이 되지 않도록 가격평균 사용
            vwap=sum((b.high+b.low+b.close)/3 for b in r)/len(r)

        low12=min(lows[-12:]) if lows else latest.close
        prior_highs=highs[-20:-1] if len(highs)>1 else highs
        resistance=max(prior_highs) if prior_highs else max(highs)
        support=max(low12, vwap*0.995)

        # support가 현재가보다 비정상적으로 위에 생기는 경우 가격 기반으로 보정
        if latest.close>0 and support>latest.close*1.02:
            support=low12
        ...
        return {
            ...
            "pullback_pct":pct(latest.close,resistance) if resistance else 0.0,
            "volume_ratio":0.0,
            "strength":latest.trade_strength or 100.0,
            "short_break":False,
            "imbalance":1.0,
            "score":50.0,
            "today_open":r[0].open if r else latest.open,
        }
    ...
    def metrics(c):
        ...
        if tot<=0:return Brain.bootstrap_metrics(c)
...
def _fred_change(label, series_id, timeout=8):
    """FRED 단일 지수 조회. (표시명, 등락률, 기준일) 반환."""
    ...
    for row in reader:
        ...
        if not raw or raw==".":
            continue
        try:
            vals.append((date,float(raw)))
        except Exception:
            continue

    if len(vals)<2:
        raise RuntimeError(f"{series_id} 데이터 부족")

    prev_date,prev=vals[-2]
    last_date,last=vals[-1]
    return label,pct(last,prev),last_date


def us_market_summary():
    """미국 3대 지수를 병렬 조회.
    일부 지수만 성공해도 표시하고, 전부 실패하면 마지막 성공 캐시를 사용한다.
    """
    cache=STORE.load_us_market() if hasattr(STORE,"load_us_market") else {}
    series=(("S&P500","SP500"),("NASDAQ","NASDAQCOM"),("DOW","DJIA"))
    ...
    try:
        # 3개 지수를 순차 15초씩 기다리지 않고 동시에 조회한다.
        with ThreadPoolExecutor(max_workers=3, thread_name_prefix="fred-us") as ex:
            futs={ex.submit(_fred_change,label,sid,8):(label,sid) for label,sid in series}
            for fut in as_completed(futs, timeout=10):
                label,sid=futs[fut]
                try:
                    name,change,data_date=fut.result()
                    rows[name]=change
                    dates[name]=data_date
                except Exception as e:
    ...
    for label,_ in series:
        if label in rows:
            ordered.append(f"{label} {rows[label]:+.2f}%")

    if ordered:
        text=" · ".join(ordered)
        data_date=min(dates.values()) if dates else "-"
        payload={
            "text":text,
        ...
        if hasattr(STORE,"save_us_market"):
            STORE.save_us_market(payload)

        # 일부 지수만 실패하면 성공한 데이터는 그대로 제공한다.
        if len(ordered)<3:
            log.warning("US market partial fetch: %s", " | ".join(errors[:3]) or "partial")
            return f"{text}\n미국장 기준일 {data_date} · 일부 지수 조회 실패"

        return f"{text}\n미국장 기준일 {data_date}"

    # 전부 실패: 마지막 성공값 폴백
    if errors:
        log.warning("US market fetch failed: %s", " | ".join(errors[:3]))

    if cache.get("text"):
        return (
            f"{cache['text']}\n"
            f"⚠️ 현재 조회 실패 · 마지막 성공값 {cache.get('time','-')}"
        )

    return "⚠️ 미국증시 현재 조회 불가 · 국내/KIS 데이터로 브리핑 진행"
...
class App:
    ...
    def sync_bars_for(self,c):
        ...
        if q:
            with STATE.lock:
                STATE.bars[c]=q
            try:
                m=BRAIN.bootstrap_metrics(c)
                t=STATE.targets.get(c)
                if m and t:
                    t.last_price=m["price"]
                    t.vwap=m["vwap"]
                    t.support=m["support"]
                    t.resistance=m["resistance"]
                    t.trade_strength=m["strength"]
                    t.volume_ratio=m["volume_ratio"]
                    t.pullback_pct=m["pullback_pct"]
                    t.score=m["score"]
                log.info(
                    "VIP minute bars ready: %s %s bars | support %.0f resistance %.0f vwap %.0f",
                    STATE.vip_targets.get(c,c),len(q),
                    t.support if t else 0,
                    t.resistance if t else 0,
                    t.vwap if t else 0,
                )
            except Exception as e:
                log.warning("initial metrics %s failed: %s",c,e)
        else:
    ...
    def main_buttons(self):
        ...
        if SETTINGS.miniapp_url:
            rows.append([{"text":"📱 뽕실 Mini App 열기","web_app":{"url":SETTINGS.miniapp_url}}])
        rows += [
    ...
    def morning(self):
        for c in list(STATE.vip_targets):
            ...
            if not t:
                continue
            if t.support<=0 or t.resistance<=0 or t.vwap<=0:
                try:
                    m=BRAIN.bootstrap_metrics(c)
                    if m:
                        t.last_price=m["price"]
                    ...
                    log.warning("morning metrics %s failed: %s",c,e)
        ...
        return (
            ...
            +"\n\n"
            +"\n".join(
                f"• <b>{html.escape(n)}</b> · 지지 {STATE.targets[c].support:,.0f} · 저항 {STATE.targets[c].resistance:,.0f}"
    ...
    def start(self):
        ...
        try:
            ...
            if SETTINGS.telegram_token:
                ...
                WORKER.submit(BOT.configure_miniapp_menu)
            else:
...
def _validate_telegram_init_data(init_data: str):
    """Telegram Mini App initData HMAC 검증. 성공 시 user dict 반환."""
    if not SETTINGS.miniapp_require_auth:
        return {"id": str(SETTINGS.chat_id or "preview"), "preview": True}
    ...
    try:
        ...
        secret=hmac.new(b"WebAppData",SETTINGS.telegram_token.encode("utf-8"),hashlib.sha256).digest()
        ...
        allowed=str(SETTINGS.chat_id).strip()
        if allowed and user and str(user.get("id"))!=allowed:
            return None
...
@web.get("/miniapp")
def miniapp():
    return send_file(str(Path(__file__).with_name("miniapp.html")))
...
def miniapp_state():
    user=_mini_auth()
    if not user:
        return jsonify({"ok":False,"error":"telegram_auth_required"}),401
    ...
    for c,n in STATE.vip_targets.items():
        ...
        vip.append({
            ...
            "name":n,
            "stage":t.stage,
            "stage_label":STATE_LABEL.get(t.stage,t.stage),
            "price":round(t.last_price or 0,2),
            "vwap":round(t.vwap or 0,2),
            "support":round(t.support or 0,2),
            "resistance":round(t.resistance or 0,2),
            "strength":round(t.trade_strength or 0,1),
            "volume_ratio":round(t.volume_ratio or 0,2),
            "pullback_pct":round(t.pullback_pct or 0,2),
            "score":round(t.score or 0,1),
            "note":t.note or "",
            "signal_count_today":t.signal_count_today,
            "last_signal_at":t.last_signal_at,
            "orderbook_imbalance":round(ob.imbalance,2) if ob else None,
            "best_ask":round(ob.best_ask,2) if ob else None,
            "best_bid":round(ob.best_bid,2) if ob else None,
    ...
    for c,p in POSITIONS.data.items():
        t=STATE.targets.get(c)
        current=(t.last_price if t and t.last_price else p.entry)
        holdings.append({
            "code":c,
            "name":p.name,
            "entry":round(p.entry,2),
            "current":round(current,2),
            "return_pct":round(pct(current,p.entry),2),
            "highest":round(p.highest,2),
            "stop":round(p.stop,2),
            "level":p.level,
    ...
    payload={
        ...
        "market":{
            "risk":STATE.market_risk,
            "kospi":round(STATE.market_changes.get("KOSPI",0),2),
            "kosdaq":round(STATE.market_changes.get("KOSDAQ",0),2),
        },
        "runtime":{
            "boot":STATE.runtime.get("boot"),
            "ws":STATE.runtime.get("ws"),
            "trade_subscribed":STATE.runtime.get("trade_subscribed",0),
            "order_subscribed":STATE.runtime.get("order_subscribed",0),
            "last_tick":STATE.runtime.get("last_tick"),
            "last_error":STATE.runtime.get("last_error"),
        },
        "vip":vip,
        "holdings":holdings,
        "signals":list(reversed(STATE.signals[-20:])),
...
def miniapp_stock(code):
    ...
    if code not in STATE.vip_targets:
        return jsonify({"ok":False,"error":"not_found"}),404

    t=STATE.targets.get(code)
    bars=list(STATE.bars.get(code,[]))
    cur=STATE.current.get(code)
    if cur:bars.append(cur)
    bars=bars[-120:]
    p=POSITIONS.data.get(code)
    ob=STATE.books.get(code)
    return _no_store(make_response(jsonify({
        "ok":True,
        "code":code,
        "name":STATE.vip_targets.get(code,code),
        "state":{
            "stage":t.stage,
            "stage_label":STATE_LABEL.get(t.stage,t.stage),
            "price":t.last_price,
        ...
        "position":{
            "entry":p.entry,
            "current":t.last_price if t and t.last_price else p.entry,
            "return_pct":pct(t.last_price if t and t.last_price else p.entry,p.entry),
            "highest":p.highest,
            "stop":p.stop,
            "level":p.level,
        } if p else None,
        "bars":[
            {
                "time":b.minute.isoformat(),
                "open":b.open,
                "high":b.high,
                "low":b.low,
                "close":b.close,
                "volume":b.volume,
                "strength":b.trade_strength,
            } for b in bars
        ],
    })))
