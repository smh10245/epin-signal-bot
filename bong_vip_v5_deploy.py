...
class Settings:
    version: str = "5.0.3 VIP Repeat Sniper Stable"
    ...
    health_log_minutes: int = env_int("HEALTH_LOG_MINUTES", 30)
...
class Store:
    def __init__(self):
        self.vip=DATA_DIR/"vip_targets.json";self.pos=DATA_DIR/"positions.json";self.sig=DATA_DIR/"signal_history.json";self.trade=DATA_DIR/"trade_history.json";self.master=DATA_DIR/"stock_master.json";self.us_market=DATA_DIR/"us_market_cache.json"
    ...
    def load_us_market(self):return self.read(self.us_market,{})
    def save_us_market(self,v):self.write(self.us_market,v)
STORE=Store()
...
def us_market_summary():
    """미국 3대 지수를 한 번에 조회하고, Rate Limit 시 마지막 성공값으로 폴백."""
    cache=STORE.load_us_market()
    if not HAS_YF:
        if cache.get("text"):
            return f"{cache['text']}\n(미국증시 실시간조회 불가 · 마지막 성공값 {cache.get('time','-')})"
        return "미국증시: yfinance 미설치"
    try:
        symbols=["^GSPC","^IXIC","^DJI"]
        labels={"^GSPC":"S&P500","^IXIC":"NASDAQ","^DJI":"DOW"}
        d=yf.download(
            tickers=symbols,
            period="5d",
            interval="1d",
        ...
        if d is not None and len(d)>=2:
            ...
            for s in symbols:
                try:
                    series=close[s].dropna() if hasattr(close,"columns") else close.dropna()
                    if len(series)>=2:
                        rows.append(f"{labels[s]} {pct(float(series.iloc[-1]),float(series.iloc[-2])):+.2f}%")
                except Exception:
                    continue
        if not rows:
            raise RuntimeError("미국증시 가격 데이터 없음")
        text=" · ".join(rows)
        STORE.save_us_market({"text":text,"time":now().strftime("%Y-%m-%d %H:%M")})
        return text
    except Exception as e:
        msg=str(e)
        log.warning("US market fetch failed: %s",msg)
        if cache.get("text"):
            return f"{cache['text']}\n⚠️ 현재 조회 제한 · 마지막 성공값 {cache.get('time','-')}"
        if "Too Many Requests" in msg or "Rate limited" in msg or "429" in msg:
            return "⚠️ 미국증시 조회 제한(Rate Limit) · 국내/KIS 데이터로 브리핑 진행"
...
class App:
    ...
    def sync_bars_for(self,c):
        ...
        if q:
            ...
            try:
                ...
                t=STATE.targets.get(c)
                if m and t:
                    t.last_price=m["price"];t.vwap=m["vwap"];t.support=m["support"];t.resistance=m["resistance"]
                    t.trade_strength=m["strength"];t.volume_ratio=m["volume_ratio"];t.pullback_pct=m["pullback_pct"];t.score=m["score"]
            except Exception as e:
                log.warning("initial metrics %s failed: %s",c,e)
            log.info(
                "VIP minute bars ready: %s %s bars | support %.0f resistance %.0f",
                STATE.vip_targets.get(c,c),len(q),
                STATE.targets.get(c).support if STATE.targets.get(c) else 0,
                STATE.targets.get(c).resistance if STATE.targets.get(c) else 0
            )
        else:
    ...
    def morning(self):
        for c in list(STATE.vip_targets):
            ...
            if not t:
                continue
            if t.support<=0 or t.resistance<=0:
                try:
                    m=BRAIN.metrics(c)
                    if m:
                        t.last_price=m["price"];t.vwap=m["vwap"];t.support=m["support"];t.resistance=m["resistance"]
    ...
    def diag(self):
        n=now();tick_rows=[]
        for c,name in STATE.vip_targets.items():
            last=STATE.last_tick_at.get(c)
            if last:
                age=max(0,int((n-last).total_seconds()));tick_rows.append(f"• {html.escape(name)} {age}초 전")
            else:
                tick_rows.append(f"• {html.escape(name)} -")
        return (
            f"🩺 <b>VIP V5 진단</b>\n"
            ...
            +"\n".join(tick_rows)
            +f"\n부팅완료 {STATE.runtime.get('boot_completed_at') or '-'}\n"
    ...
    def health_log(self):
        try:
            ...
            for c,name in STATE.vip_targets.items():
                last=STATE.last_tick_at.get(c)
                if last:
                    age=max(0,int((n-last).total_seconds()));parts.append(f"{name}:{age}s")
                else:
                    parts.append(f"{name}:-")
            log.info(
                "[VIP-HEALTH] boot=%s ws=%s trade=%s order=%s VIP=%s/%s last_tick=%s | %s",
                STATE.runtime.get("boot"),STATE.runtime.get("ws"),
                STATE.runtime.get("trade_subscribed"),STATE.runtime.get("order_subscribed"),
                len(STATE.vip_targets),STATE.vip_limit,
                STATE.runtime.get("last_tick") or "-"," / ".join(parts)
            )
        except Exception as e:
    ...
    def scheduler(self):
        ...
        while True:
            ...
            if time.time()-last_health_log>=max(5,SETTINGS.health_log_minutes)*60:
                last_health_log=time.time();self.health_log()
            if n.weekday()<5:
