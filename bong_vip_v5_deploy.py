...
class Settings:
    version: str = "5.2.0 Myeongha Dual Entry Sniper"
...
STATE_LABEL={ST_WAIT:"⚪ 대기",ST_PULLBACK:"🟡 눌림관찰",ST_READY:"🟠 재진입준비",ST_MOMENTUM:"🔵 상승초입",ST_SIGNAL:"🟢 재진입타점",ST_BREAKOUT:"🚀 상승진입타점",ST_HOLD:"💼 보유관리",ST_COOLDOWN:"⏳ 쿨다운"}
...
class PositionEngine:
    ...
    def check(self,c,current):
        ...
        hard_exit=(current<=p.stop) or (exit_score>=7 and gain>0)
...
class Brain:
    ...
    def metrics(c):
        ...
        # 상승 지속형 구조: HH-HL + 연속 종가 상승 + 직전 고점 돌파의 초기/재가속만 포착
        closes=[b.close for b in r]
        hh_hl=False
        rising_closes=False
        if len(r)>=7:
            prev_hi=max(b.high for b in r[-7:-4])
            curr_hi=max(b.high for b in r[-4:-1])
            prev_lo=min(b.low for b in r[-7:-4])
            curr_lo=min(b.low for b in r[-4:-1])
        ...
        return {
            ...
            "short_break":br,"imbalance":imb,"score":score,"today_open":bars[0].open if bars else r[0].open,
            "projected_volume":projected,"projected_volume_ratio":projected_ratio,
            "volume_burst":volume_burst,"elapsed_seconds":elapsed,
            "hh_hl":hh_hl,"rising_closes":rising_closes,
            "vwap_gap_pct":vwap_gap,"breakout_extension_pct":breakout_extension,
    ...
    def evaluate(self,c):
        ...
        not_overheated=(
            0.0<=m["vwap_gap_pct"]<=SETTINGS.breakout_vwap_max_pct
            and 0.0<=m["breakout_extension_pct"]<=SETTINGS.breakout_extension_max_pct
        )
        momentum=(
            m["price"]>m["vwap"]
            and m["hh_hl"]
            and m["rising_closes"]
            and m["strength"]>=SETTINGS.momentum_strength_min
            and m["imbalance"]>=0.95
            and daily_context
            and m["vwap_gap_pct"]<=SETTINGS.breakout_vwap_max_pct
        )
        breakout=(
            momentum
            and m["short_break"]
            and m["strength"]>=SETTINGS.breakout_strength_min
            and (m["projected_volume_ratio"]>=SETTINGS.breakout_volume_ratio or m["volume_ratio"]>=1.10)
            and m["imbalance"]>=SETTINGS.breakout_orderbook_min
            and not_overheated
        )
        ...
        if breakout:
            if STATE.market_risk=="CAUTION" and m["score"]<82:
                return ST_MOMENTUM,m,"시장 CAUTION - 상승 확인 강화"
            return ST_BREAKOUT,m,(
                f"HH-HL + 고점돌파 + 체결강도 {m['strength']:.0f} + "
                f"거래량 재가속 {max(m['projected_volume_ratio'],m['volume_ratio']):.2f}배"
            )
        ...
        if momentum:
            return ST_MOMENTUM,m,(
                f"상승초입 감지 · HH-HL · 체결강도 {m['strength']:.0f} · "
                f"VWAP 이격 {m['vwap_gap_pct']:+.2f}%"
            )

        if m["vwap_gap_pct"]>SETTINGS.breakout_vwap_max_pct or m["breakout_extension_pct"]>SETTINGS.breakout_extension_max_pct:
            return ST_WAIT,m,"상승 중이지만 이격 과열 - 추격 금지"
...
class App:
    ...
    def evaluate_target(self,c):
        ...
        if st in (ST_SIGNAL,ST_BREAKOUT) and (changed or self.allowed(c,"signal",10)):
            t.last_signal_at=now().isoformat();t.signal_count_today+=1
            signal_type="BREAKOUT" if st==ST_BREAKOUT else "REENTRY"
            STATE.signals.append({
                "code":c,"name":t.name,"time":t.last_signal_at,"price":t.last_price,
                "support":t.support,"resistance":t.resistance,"vwap":t.vwap,
                "strength":t.trade_strength,"score":t.score,
                "projected_volume_ratio":t.projected_volume_ratio,
                "ma5d":t.ma5d,"ma20d":t.ma20d,"signal_type":signal_type
            })
            STORE.save_signals(STATE.signals)
            WORKER.submit(self.send_signal_alert,c)
            self.stage_alert[(c,"signal")]=now()

        elif changed and st==ST_MOMENTUM and self.allowed(c,"momentum",20):
            BOT.send_async(
                f"🔵 <b>{html.escape(t.name)} 상승초입 감지</b>\n"
                f"현재 {t.last_price:,.0f} · VWAP {t.vwap:,.0f}\n"
                f"체결강도 {t.trade_strength:.0f} · 거래량비 {t.volume_ratio:.2f}\n"
                f"아직 상승진입타점 아님 · 고점돌파/거래량 재가속 확인 대기"
            )
            self.stage_alert[(c,"momentum")]=now()

        elif changed and st==ST_PULLBACK and self.allowed(c,"pull",30):
            BOT.send_async(
                f"🟡 <b>{html.escape(t.name)} 눌림관찰</b>\n"
                f"현재 {t.last_price:,.0f} · VWAP {t.vwap:,.0f} · 지지 {t.support:,.0f}\n"
                f"거래량비 {t.volume_ratio:.2f} · 체결강도 {t.trade_strength:.0f}\n"
                f"아직 매수신호 아님"
            )
    ...
    def signal_text(self,t):
        ...
        if t.stage==ST_BREAKOUT:
            return (
                f"🚀 <b>[명하 VIP 상승진입 타점] {html.escape(t.name)}</b>\n"
                f"현재 <b>{t.last_price:,.0f}원</b>\n"
                f"VWAP {t.vwap:,.0f} · 직전저항 {t.resistance:,.0f}\n"
                f"체결강도 {t.trade_strength:.0f} · 거래량비 {t.volume_ratio:.2f}{proj} · 점수 {t.score:.0f}{obs}{daily}\n\n"
                f"✅ HH-HL 상승구조\n✅ 직전 고점 돌파\n✅ 체결강도 + 거래량 재가속\n✅ 과도한 추격 이격 아님\n"
                f"⚠️ 실제 주문은 사용자가 직접 수행"
            )
        return (
            f"🟢 <b>[명하 VIP 재진입 타점] {html.escape(t.name)}</b>\n"
            f"현재 <b>{t.last_price:,.0f}원</b>\n"
            f"VWAP {t.vwap:,.0f} · 지지 {t.support:,.0f} · 저항 {t.resistance:,.0f}\n"
            f"체결강도 {t.trade_strength:.0f} · 거래량비 {t.volume_ratio:.2f}{proj} · 점수 {t.score:.0f}{obs}{daily}\n\n"
            f"✅ 눌림/추세 맥락 확인\n✅ 체결강도 + 단기고점 돌파\n✅ 거래량 재유입/선제증가 확인\n"
            f"⚠️ 실제 주문은 사용자가 직접 수행"
        )
        ...
        for i in root.findall(".//item")[:limit]:
            ...
            if pub_raw:
                try:
                    ...
                    pub_text=f"{kst.year}년 {kst.month}월 {kst.day}일({weekdays[kst.weekday()]}) {ampm} {hour12}시 {kst.minute:02d}분"
                except Exception:
                    pub_text=pub_raw
            out.append({
                "title":title,
...
def miniapp_news(code):
    ...
    return _no_store(make_response(jsonify({"ok":True,"code":code,"name":t.name,"checked_at":now().isoformat(),
        "news":[{"title":r.get("title",""),"published":r.get("published"),"time_text":r.get("time_text") or r.get("published") or "시간정보 없음"} for r in rows]})))
