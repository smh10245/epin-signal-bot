...
class Settings:
    version: str = "5.0.1 VIP Repeat Sniper Stable"
...
class MasterIndex:
    ...
    def load(self):
        ...
        log.info("KIS stock master ready: %s symbols",len(rows))
    def resolve(self,q):
        s=self._norm(q)
        if s.isdigit():
...
class KIS:
    ...
    def stream(self,on_tick,on_book):
        with self.stream_lock:
            if self.stream_running:log.warning("NXT stream duplicate start blocked");return
            self.stream_running=True
        STATE.runtime["ws_engine"]="running";log.info("NXT stream engine started")
        ...
        try:
            while True:
                if SETTINGS.kis_env!="real":
                    ...
                    if last_session_state!="virtual":log.warning("KIS_ENV is not real - NXT realtime disabled");last_session_state="virtual"
                ...
                if not in_session(SETTINGS.nxt_start,SETTINGS.nxt_end):
                    ...
                    if last_session_state!="waiting":log.info("NXT outside session (%s-%s) - waiting",SETTINGS.nxt_start,SETTINGS.nxt_end);last_session_state="waiting"
                    time.sleep(30);continue
                if last_session_state!="active":log.info("NXT session active - websocket connection starting");last_session_state="active"
...
def google_news_headlines(q,limit=4):
    try:
        url=f"https://news.google.com/rss/search?q={urllib.parse.quote(q)}&hl=ko&gl=KR&ceid=KR:ko";r=requests.get(url,timeout=10,headers={"User-Agent":"Mozilla/5.0"});r.raise_for_status();root=ET.fromstring(r.text);return [((i.findtext("title") or "").strip()) for i in root.findall(".//item")[:limit] if (i.findtext("title") or "").strip()]
...
class App:
    ...
    def sync_bars_for(self,c):
        ...
        for r in reversed(raw):
            ...
            except Exception as e:log.debug("minute bar parse failed %s: %s",c,e)
        if q:
            with STATE.lock:STATE.bars[c]=q
            log.info("VIP minute bars ready: %s %s bars",STATE.vip_targets.get(c,c),len(q))
        else:log.warning("VIP minute bars empty: %s",STATE.vip_targets.get(c,c))
    ...
    def news_text(self,c=None):
        ...
        for x in codes:
            n=STATE.vip_targets.get(x,x);hs=google_news_headlines(f"{n} 주식 수주 공시",4);lines.append(f"\n<b>{html.escape(n)}</b>\n"+("\n".join("• "+html.escape(h) for h in hs) if hs else "헤드라인 없음"))
        return "\n".join(lines)
    ...
    def start(self):
        with self.start_lock:
            if self.started:log.warning("APP.start duplicate call blocked");return
            self.started=True
        STATE.runtime["boot"]="starting";STATE.runtime["boot_stage"]="worker";log.info("VIP V5 APP.start entered")
        try:
            WORKER.start();log.info("Async worker started")
            STATE.runtime["boot_stage"]="master"
            try:
                MASTER.load();STATE.runtime["master"]=f"ready:{len(MASTER.code_to_name)}";log.info("Master initialization complete")
            except Exception as e:
                STATE.runtime["master"]="failed";STATE.runtime["last_error"]=f"master: {e}";log.exception("master load failed - continuing with stored VIP codes")
            ...
            STATE.save_vip();POSITIONS.load();log.info("VIP/position restore complete: VIP %s / positions %s",len(STATE.vip_targets),len(POSITIONS.data))
            STATE.runtime["boot_stage"]="telegram";BOT.text_handler=self.handle_text;BOT.callback_handler=self.handle_cb
            if SETTINGS.telegram_token:
                threading.Thread(target=BOT.poll,daemon=True,name="telegram-poll").start();STATE.runtime["telegram"]="polling";log.info("Telegram polling thread started")
            else:STATE.runtime["telegram"]="disabled_missing_token";log.warning("Telegram token missing")
            STATE.runtime["boot_stage"]="bars"
            for c in list(STATE.vip_targets):WORKER.submit(self.sync_bars_for,c)
            log.info("Initial minute-bar sync queued for %s VIP symbols",len(STATE.vip_targets))
            STATE.runtime["boot_stage"]="scheduler";threading.Thread(target=self.scheduler,daemon=True,name="scheduler").start();STATE.runtime["scheduler"]="running";log.info("Scheduler started")
            STATE.runtime["boot_stage"]="nxt"
            if SETTINGS.kis_app_key and SETTINGS.kis_app_secret:
                threading.Thread(target=KIS_CLIENT.stream,args=(self.on_tick,self.on_book),daemon=True,name="KIS-NXT-websocket").start();STATE.runtime["ws_engine"]="starting";log.info("NXT websocket engine thread started")
            else:
                STATE.runtime["ws"]="disabled_missing_credentials";STATE.runtime["ws_engine"]="disabled_missing_credentials";STATE.runtime["last_error"]="KIS_APP_KEY/KIS_APP_SECRET missing";log.error("KIS credentials missing - NXT disabled")
            STATE.runtime["boot"]="ready";STATE.runtime["boot_stage"]="complete";STATE.runtime["boot_completed_at"]=now().isoformat();log.info("VIP V5 boot complete")
            BOT.send_async(f"🚀 <b>뽕실 VIP V{SETTINGS.version}</b>\nVIP {len(STATE.vip_targets)}/{STATE.vip_limit} · 집중호가 {SETTINGS.focus_orderbook_slots}\n반복매매 쿨다운 {SETTINGS.cooldown_minutes}분\nNXT 상태 {STATE.runtime['ws']}\n자동주문 없음",buttons=self.main_buttons())
        except Exception as e:
            STATE.runtime["boot"]="failed";STATE.runtime["boot_stage"]="fatal";STATE.runtime["last_error"]=f"startup fatal: {e}";log.exception("VIP V5 startup fatal error")
...
def delayed_start():
    try:
        STATE.runtime["boot"]="waiting";STATE.runtime["boot_stage"]="render_delay"
        ...
        log.info("Render wait complete - calling APP.start");APP.start()
    except Exception as e:
        STATE.runtime["boot"]="failed";STATE.runtime["boot_stage"]="delayed_start_fatal";STATE.runtime["last_error"]=f"delayed_start: {e}";log.exception("delayed_start fatal error")
