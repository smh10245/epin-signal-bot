...
class Settings:
    version: str = "5.0.0 VIP Repeat Sniper"
...
def miniapp_state():
    ...
    for c,p in POSITIONS.data.items():
        ...
        holdings.append({"code":c,"name":p.name,"entry":round(p.entry,2),"current":round(current,2),
            "return_pct":round(pct(current,p.entry),2),"highest":round(p.highest,2),"stop":round(p.stop,2),
            "level":p.level,"opened_at":p.opened_at})
...
def miniapp_stock(code):
    ...
    return _no_store(make_response(jsonify({"ok":True,"code":code,"name":STATE.vip_targets.get(code,code),
        ...
        "position":{"entry":p.entry,"current":t.last_price if t.last_price else p.entry,
                    "return_pct":pct(t.last_price if t.last_price else p.entry,p.entry),"highest":p.highest,
                    "stop":p.stop,"level":p.level} if p else None,
