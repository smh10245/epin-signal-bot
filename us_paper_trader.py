...
class USPaperService:
    ...
    def __init__(
        ...
        day_entry_start="10:00", day_entry_end="15:00",
        swing_entry_start="15:15", swing_entry_end="15:45",
        day_min_rvol=1.50, day_min_score=74.0,
        swing_min_rvol=1.50, swing_min_score=80.0,
        breakeven_trigger_r=1.0, day_trail_trigger_r=1.5, swing_trail_trigger_r=2.0,
        symbol_loss_cooldown_minutes=60, symbol_normal_cooldown_minutes=30,
        ...
        self.day_entry_start=_hm(day_entry_start,dtime(10,0))
        self.day_entry_end=_hm(day_entry_end,dtime(15,0))
        self.swing_entry_start=_hm(swing_entry_start,dtime(15,15))
        self.swing_entry_end=_hm(swing_entry_end,dtime(15,45))
        self.day_min_rvol=float(day_min_rvol);self.day_min_score=float(day_min_score)
        self.swing_min_rvol=float(swing_min_rvol);self.swing_min_score=float(swing_min_score)
        self.breakeven_trigger_r=max(0.5,float(breakeven_trigger_r))
        self.day_trail_trigger_r=max(self.breakeven_trigger_r,float(day_trail_trigger_r))
        self.swing_trail_trigger_r=max(self.breakeven_trigger_r,float(swing_trail_trigger_r))
        self.symbol_loss_cooldown_minutes=max(5,int(symbol_loss_cooldown_minutes))
        self.symbol_normal_cooldown_minutes=max(0,int(symbol_normal_cooldown_minutes))
        self.max_symbol_losses_per_day=max(1,int(max_symbol_losses_per_day))
        ...
        self.state={
            ...
            "cooldowns":{},"symbol_day_stats":{},"strategy_version":"HYBRID_1.3",
        }
        self._load()
        ...
        self.state.setdefault("symbol_day_stats",{})
        self.state.setdefault("strategy_version","HYBRID_1.3")
    ...
    @classmethod
    def _trading_days_between(cls,start_date,end_date):
        if isinstance(start_date,str):
            try:start_date=date.fromisoformat(start_date)
            except Exception:return 0
        if isinstance(end_date,str):
            try:end_date=date.fromisoformat(end_date)
            except Exception:return 0
        if not start_date or not end_date or end_date<start_date:return 0
        n=0;d=start_date
        while d<=end_date:
            if d.weekday()<5 and not cls._is_market_holiday(d):n+=1
            d+=timedelta(days=1)
        return n

    def _cooldown_blocked(self,symbol):
        now_et=self._now_et()
        stats=(self.state.get("symbol_day_stats") or {}).get(symbol) or {}
        if stats.get("date")==str(now_et.date()) and int(stats.get("losses",0))>=self.max_symbol_losses_per_day:
            return True,"DAILY_SYMBOL_LOSS_LIMIT"
        raw=(self.state.get("cooldowns") or {}).get(symbol)
        if raw:
            try:
                until=datetime.fromisoformat(raw)
                if until.tzinfo is None:until=until.replace(tzinfo=NY)
                if now_et<until.astimezone(NY):return True,"COOLDOWN"
            except Exception:pass
        return False,""

    def _set_cooldown_after_exit(self,symbol,net_pnl_usd):
        now_et=self._now_et()
        stats=self.state.setdefault("symbol_day_stats",{}).get(symbol) or {}
        if stats.get("date")!=str(now_et.date()):
            stats={"date":str(now_et.date()),"losses":0,"wins":0}
        if net_pnl_usd<0:
            stats["losses"]=int(stats.get("losses",0))+1
            if stats["losses"]>=self.max_symbol_losses_per_day:
                close=self._regular_close(now_et.date())
                until=now_et.replace(hour=close.hour,minute=close.minute,second=0,microsecond=0)
            else:
                until=now_et+timedelta(minutes=self.symbol_loss_cooldown_minutes)
        else:
            stats["wins"]=int(stats.get("wins",0))+1
            until=now_et+timedelta(minutes=self.symbol_normal_cooldown_minutes)
        self.state["symbol_day_stats"][symbol]=stats
    ...
    def _feature_one(self,symbol,intra,daily,et):
        ...
        try:
            ...
            ma20=0.0;ma50=0.0
            try:
                closes_daily=daily["Close"].dropna().astype(float)
                if len(closes_daily)>=20:ma20=float(closes_daily.tail(20).mean())
                if len(closes_daily)>=50:ma50=float(closes_daily.tail(50).mean())
            except Exception:pass
    ...
    def _entry_signal(self,c:USCandidate,intra):
        ...
        blocked,why=self._cooldown_blocked(c.symbol)
        if blocked:return False,why
        t=et.time()
        if c.atr<=0 or c.vwap<=0:return False,"DATA"
        if c.price<c.vwap:return False,"BELOW_VWAP"
        if c.gap_pct<=0:return False,"NEGATIVE_GAP"
        if c.price-c.vwap>0.75*c.atr:return False,"EXTENDED_VWAP"
        if c.premarket_high>0 and c.price>c.premarket_high+0.35*c.atr:return False,"EXTENDED_PM_HIGH"
        try:
            idx=intra.index
            local=idx.tz_convert(NY) if getattr(idx,"tz",None) else idx.tz_localize(NY)
            f=intra.copy();f.index=local
            ...
            if len(reg)<5:return False,"BARS"
            last=reg.tail(6)
            closes=[_f(x) for x in last["Close"]]
            lows=[_f(x) for x in last["Low"]]
            highs=[_f(x) for x in last["High"]]
            vols=[_f(x) for x in last["Volume"]]
            rising=closes[-1]>closes[-2]>=closes[-3]
            medv=median(vols[:-1]) if len(vols)>2 else 0
            vol_confirm=(vols[-1]>=medv*1.10) if medv>0 else True
            if t<self.day_entry_start:return False,"OPENING_NO_TRADE"
            if self.day_entry_start<=t<=self.day_entry_end:
                if c.rvol<self.day_min_rvol or c.score<self.day_min_score:return False,"DAY_QUALITY_FILTER"
                touched_vwap=min(lows[-4:])<=c.vwap*1.003 and closes[-1]>=c.vwap*1.002
                pm_break=(c.premarket_high>0 and closes[-1]>=c.premarket_high and closes[-2]<c.premarket_high*1.002)
                if pm_break and vol_confirm:return True,"DAY_PM_HIGH_BREAK"
                if touched_vwap and rising and vol_confirm:return True,"DAY_VWAP_RECLAIM"
                return False,"DAY_WAIT"
            if self.swing_entry_start<=t<=self.swing_entry_end:
                if c.rvol<self.swing_min_rvol or c.score<self.swing_min_score:return False,"SWING_QUALITY_FILTER"
                daily_trend=(c.ma20>0 and c.price>c.ma20 and (c.ma50<=0 or c.ma20>=c.ma50*0.995))
                near_high=(c.day_high>0 and c.price>=c.day_high*0.985)
                trend_5m=closes[-1]>closes[-2]>closes[-3] and closes[-1]>=max(highs[-4:-1])*0.998
                if daily_trend and near_high and trend_5m and vol_confirm:return True,"SWING_STRONG_CLOSE"
                return False,"SWING_WAIT"
    ...
    def _queue_buy(self,c,reason):
        ...
        blocked,_=self._cooldown_blocked(c.symbol)
        if blocked:return
        eq=self._equity_usd()
        ...
        trade_type="SWING" if str(reason).startswith("SWING_") else "DAY"
        if trade_type=="SWING":
            raw_stop=min(c.price-1.45*c.atr,c.vwap-0.55*c.atr)
            stop=max(raw_stop,c.price*0.955)
            stop=min(stop,c.price*0.985)
        else:
            raw_stop=min(c.price-0.95*c.atr,c.vwap-0.30*c.atr)
            stop=max(raw_stop,c.price*0.98)
            stop=min(stop,c.price*0.994)
        ...
        self.state["pending_orders"][c.symbol]={
            "side":"BUY","symbol":c.symbol,"qty":qty,"reason":reason,"trade_type":trade_type,
            ...
            "stop_hint":stop,"initial_risk_hint":risk_per_share,
        }
        self._log("US_ORDER_QUEUED",side="BUY",symbol=c.symbol,qty=qty,reason=reason,trade_type=trade_type)
    ...
    def _process_pending(self,c):
        ...
        if side=="BUY":
            ...
            if gross+cost>self.state["cash_usd"]:
                ...
                gross=fill*qty;cost=gross*(self.buy_fee_pct+self.fx_cost_pct)/100
            trade_type=str(o.get("trade_type") or ("SWING" if str(o.get("reason","")).startswith("SWING_") else "DAY"))
            stop=_f(o.get("stop_hint"))
            if stop<=0:stop=fill-(1.45*c.atr if trade_type=="SWING" else 0.95*c.atr)
            initial_risk=max(0.01,fill-stop)
            self.state["cash_usd"]-=gross+cost
            self.state["positions"][c.symbol]={
                "symbol":c.symbol,"name":c.name,"qty":qty,"entry":fill,"stop":stop,
                "initial_stop":stop,"initial_risk":initial_risk,
                "highest":fill,"opened_at":self.now_fn().isoformat(),"buy_cost":cost,
                "entry_reason":o["reason"],"trade_type":trade_type,
                "entry_slippage_pct":sp,"last_price":c.price,"atr_entry":c.atr,
                "breakeven_armed":False,"trail_armed":False,
            }
    ...
    def _sell(self,symbol,c,reason):
        ...
        hold_minutes=None
        try:hold_minutes=round((self.now_fn()-datetime.fromisoformat(p["opened_at"])).total_seconds()/60,1)
        except Exception:pass
        initial_risk=max(0.01,_f(p.get("initial_risk"),float(p["entry"])-_f(p.get("initial_stop"))))
        r_mult=(fill-float(p["entry"]))/initial_risk if initial_risk>0 else 0.0
        r={
    ...
    def _manage_position(self,c):
        ...
        if not p:return
        p["last_price"]=c.price
        p["highest"]=max(float(p.get("highest") or c.price),c.price)

        entry=float(p["entry"])
        trade_type=str(p.get("trade_type") or "DAY")
        atr=max(c.atr,float(p.get("atr_entry") or 0))
        initial_risk=max(0.01,float(p.get("initial_risk") or (entry-float(p.get("initial_stop") or p["stop"]))))
        stop=float(p["stop"])
        r_now=(c.price-entry)/initial_risk
        r_high=(float(p["highest"])-entry)/initial_risk

        if r_high>=self.breakeven_trigger_r:
            p["breakeven_armed"]=True
            stop=max(stop,entry+0.05*initial_risk)

        if trade_type=="SWING":
            if r_high>=self.swing_trail_trigger_r:
                p["trail_armed"]=True
                stop=max(stop,float(p["highest"])-1.15*atr)
        else:
            if r_high>=self.day_trail_trigger_r:
                p["trail_armed"]=True
                stop=max(stop,float(p["highest"])-0.75*atr)

        p["stop"]=stop
        p["r_now"]=round(r_now,3)
        p["r_high"]=round(r_high,3)

        reasons=[]
        exit_score=0
        if c.price<=stop:
            exit_score+=5;reasons.append("RISK_STOP")

        if trade_type=="DAY":
            if c.vwap and c.price<c.vwap and r_now<0:
                exit_score+=2;reasons.append("VWAP_LOSS")
            if c.rvol>=1.5 and c.price<float(p["highest"])-0.80*atr:
                exit_score+=2;reasons.append("SELL_PRESSURE")
            if c.premarket_low and c.price<c.premarket_low:
                exit_score+=3;reasons.append("PM_LOW_BREAK")
            if r_now>=2.5:
                exit_score+=5;reasons.append("DAY_TARGET_2.5R")
        else:
            if c.ma20>0 and c.price<c.ma20 and r_now<0:
                exit_score+=3;reasons.append("MA20_LOSS")
            # No fixed maximum holding period for SWING.

        et=self._now_et()
        close=self._regular_close(et.date())
        close_dt=et.replace(hour=close.hour,minute=close.minute,second=0,microsecond=0)

        # DAY -> SWING promotion near close.
        if trade_type=="DAY" and et>=close_dt-timedelta(minutes=10):
            profitable=(c.price>entry)
            above_vwap=(c.vwap>0 and c.price>c.vwap)
            strong_rvol=(c.rvol>=self.swing_min_rvol)
            near_day_high=(c.day_high>0 and c.price>=c.day_high*0.985)
            daily_trend=(c.ma20>0 and c.price>c.ma20 and (c.ma50<=0 or c.ma20>=c.ma50*0.995))

            if profitable and above_vwap and strong_rvol and near_day_high and daily_trend:
                p["trade_type"]="SWING"
                p["promoted_from_day"]=True
                p["promoted_at"]=self.now_fn().isoformat()
                p["promotion_reason"]="DAY_TO_SWING_STRONG_CLOSE"
                trade_type="SWING"
                self._log(
                    "US_DAY_PROMOTED_TO_SWING",
                    symbol=c.symbol,
                    price=round(c.price,4),
                    rvol=round(c.rvol,3),
                    r_now=round(r_now,3),
                    vwap=round(c.vwap,4),
                    day_high=round(c.day_high,4),
                    ma20=round(c.ma20,4),
                    ma50=round(c.ma50,4),
                )
            else:
                exit_score+=5
                reasons.append("DAY_EOD_WEAK_CLOSE")
        ...
        if exit_score>=5 and c.symbol not in self.state["pending_orders"]:
            ...
            self._log("US_ORDER_QUEUED",side="SELL",symbol=c.symbol,reason=reasons,trade_type=trade_type)
    ...
    def focus_update(self):
        ...
        with self.lock:
            ...
            if self.state.get("last_trade_date")!=today:
                ...
                self.state["symbol_day_stats"]={k:v for k,v in (self.state.get("symbol_day_stats") or {}).items() if v.get("date")==today}
    ...
    def summary(self):
        with self.lock:
            ...
            return {
                ...
                "symbol_day_stats":self.state.get("symbol_day_stats",{}),
                "strategy_version":self.state.get("strategy_version","HYBRID_1.3"),
                "strategy_note":"US V1.3 HYBRID: DAY는 장마감 10분 전 강한 종가 조건(수익권·VWAP 위·강한 RVOL·당일고점 근접·일봉상승추세)을 모두 유지하면 SWING으로 자동 승격, 그렇지 않으면 청산. SWING은 고정 최대보유기간 없이 추세/리스크 조건 또는 사용자가 정한 시즌 종료까지 보유.",
                ...
                "load_guard":{"universe":len(self.universe),"candidate_limit":self.candidate_limit,
                              ...
                              "day_entry_window":f"{self.day_entry_start.strftime('%H:%M')}-{self.day_entry_end.strftime('%H:%M')}",
                              "swing_entry_window":f"{self.swing_entry_start.strftime('%H:%M')}-{self.swing_entry_end.strftime('%H:%M')}",
                              "breakeven_trigger_r":self.breakeven_trigger_r,
                              "max_symbol_losses_per_day":self.max_symbol_losses_per_day},
            }
