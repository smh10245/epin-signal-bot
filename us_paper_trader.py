...
class USPaperService:
    ...
    - 전체 미국 종목을 실시간 감시하지 않고, 설정된 유동성 Universe를 저빈도로 스캔한 뒤
      상위 후보만 1분봉으로 정밀감시.
    - Gap은 참고점수로 사용하고 RVOL / Premarket H/L / VWAP / ATR / 시간대별 전략을 사용.
    - +1R 전 구조손절 유지, +1R 본전보호, +1.5R 이후 추세형 트레일링.
    - 장 후반 강한 DAY 포지션은 SWING으로 승격하며 SWING에는 고정 최대보유기간을 두지 않음.
    - 재진입 쿨다운 / 종목별 일일 진입횟수 제한 / 진입거부 진단을 기록.
    - 실제 증권사 주문 API 호출 없음.
    """
    ...
    def __init__(
        ...
        reentry_cooldown_minutes=30, max_entries_per_symbol_day=2,
        breakeven_r=1.0, trail_start_r=1.5, weak_confirm_seconds=60,
        ...
        self.max_entries_per_symbol_day=max(1,int(max_entries_per_symbol_day))
        ...
        self.state={
            ...
            "last_exit_at":{},"daily_entry_counts":{},
        }
        self._load()
        ...
        self.state.setdefault("last_exit_at",{})
        self.state.setdefault("daily_entry_counts",{})
    ...
    def _migrate_positions(self):
        """구버전 US 포지션을 V1.4 R/스윙 관리 필드로 안전하게 보강."""
        for p in self.state.get("positions",{}).values():
            entry=_f(p.get("entry"))
            atr=max(0.01,_f(p.get("atr_entry")))
            stop=_f(p.get("stop"), entry-1.15*atr if entry else 0.0)
            initial_stop=_f(p.get("initial_stop"),stop)
            p.setdefault("initial_stop",initial_stop)
            p.setdefault("initial_risk",max(0.01,entry-initial_stop) if entry else 0.01)
    ...
    def _diag(self,reason):
        ...
        self.entry_diagnostics[key]=int(self.entry_diagnostics.get(key,0))+1

    def _entry_allowed(self,symbol):
        today=str(self._now_et().date())
        counts=self.state.setdefault("daily_entry_counts",{})
        day_rows=counts.setdefault(today,{})
        if int(day_rows.get(symbol,0))>=self.max_entries_per_symbol_day:
            self._diag("DAILY_ENTRY_LIMIT")
            return False

        last_exit=self.state.setdefault("last_exit_at",{}).get(symbol)
        if last_exit:
            try:
                dt=datetime.fromisoformat(str(last_exit))
                cur=self.now_fn()
                if dt.tzinfo is None and cur.tzinfo is not None:
                    dt=dt.replace(tzinfo=cur.tzinfo)
                age=(cur-dt).total_seconds()
                if age<self.reentry_cooldown_minutes*60:
                    self._diag("REENTRY_COOLDOWN")
                    return False
            except Exception:
                pass
        return True
    ...
    def _feature_one(self,symbol,intra,daily,et):
        ...
        try:
            ...
            # V1.4: Gap은 참고요소다. 지나치게 큰 갭만 제외하고
            # 보합/소폭 갭하락 뒤 장중 강세전환도 후보로 허용한다.
            gap_abs=abs(gap)
            if gap_abs>self.max_gap_pct:return None
    ...
    def _entry_signal(self,c:USCandidate,intra):
        ...
        if c.rvol<self.min_rvol:return False,"RVOL_LOW"
        if c.score<70:return False,"SCORE_LOW"
    ...
    def _queue_buy(self,c,reason):
        ...
        if not self._entry_allowed(c.symbol):return
        eq=self._equity_usd()
        day_start=float(self.state.get("day_start_equity_usd") or eq)
        if day_start and _pct(eq,day_start)<=-abs(self.max_daily_loss_pct):
            self._diag("DAILY_LOSS_LIMIT")
            return
        if len(self.state["positions"])>=self.max_positions:
            self._diag("MAX_POSITIONS")
            return
    ...
    def _process_pending(self,c):
        ...
        if side=="BUY":
            ...
            self.state["positions"][c.symbol]={
                ...
                "atr_entry":c.atr,"trade_type":"DAY","weak_since":None,"r_multiple":0.0,
            }
            today=str(self._now_et().date())
            counts=self.state.setdefault("daily_entry_counts",{})
            counts.setdefault(today,{})
            counts[today][c.symbol]=int(counts[today].get(c.symbol,0))+1
            self._log("US_BUY_FILLED",symbol=c.symbol,qty=qty,fill=round(fill,4),
                      reason=o["reason"],initial_stop=round(stop,4),initial_risk=round(initial_risk,4))
        else:
    ...
    def _sell(self,symbol,c,reason):
        ...
        initial_risk=max(0.01,_f(p.get("initial_risk"),0.01))
        r={
            ...
            "trade_type":p.get("trade_type","DAY"),
            "initial_risk":round(initial_risk,4),
            "r_multiple_exit":round((fill-float(p["entry"]))/initial_risk,3),
        }
        self.state["trades"].append(r);self.state["positions"].pop(symbol,None)
        self.state.setdefault("last_exit_at",{})[symbol]=self.now_fn().isoformat()
        self._log("US_SELL_FILLED",**r)
        return r

    def _should_promote_to_swing(self,c,p):
        """장 마감 20분 전부터 DAY의 익일 보유 적합성을 평가."""
        et=self._now_et()
        close=self._regular_close(et.date())
        close_dt=et.replace(hour=close.hour,minute=close.minute,second=0,microsecond=0)
        if et<close_dt-timedelta(minutes=20):return False
        if c.vwap<=0 or c.price<=c.vwap:return False
        if c.score<78:return False
        if c.rvol<1.25:return False
        if c.day_high>0 and c.price<c.day_high*0.975:return False
        return True

    def _queue_sell(self,c,p,reason):
        if c.symbol in self.state["pending_orders"]:return False
        self.state["pending_orders"][c.symbol]={
            "side":"SELL","symbol":c.symbol,"qty":int(p["qty"]),
            "reason":reason,"signal_price":c.price,"created_at":self.now_fn().isoformat(),
            "execute_after":(self.now_fn()+timedelta(seconds=self.execution_poll_seconds)).isoformat(),
            "expires_at":(self.now_fn()+timedelta(seconds=self.pending_expire_seconds)).isoformat(),
        }
        self._log("US_ORDER_QUEUED",side="SELL",symbol=c.symbol,reason=reason)
        return True
    ...
    def _manage_position(self,c):
        ...
        entry=float(p["entry"])
        initial_stop=_f(p.get("initial_stop"),_f(p.get("stop"),entry-1.15*max(c.atr,0.01)))
        initial_risk=max(0.01,_f(p.get("initial_risk"),entry-initial_stop))
        r_mult=(c.price-entry)/initial_risk
        p["r_multiple"]=round(r_mult,3)

        stop=_f(p.get("stop"),initial_stop)

        # +1R 전에는 최초 구조손절을 유지한다.
        if r_mult<self.breakeven_r:
            stop=max(stop,initial_stop)

        # +1R부터 왕복 비용을 포함한 본전보호를 시작한다.
        if r_mult>=self.breakeven_r:
            roundtrip_cost_pct=(
                self.buy_fee_pct+self.sell_fee_pct
                +(2*self.fx_cost_pct)+(2*self.base_slippage_pct)
            )
            breakeven_stop=entry*(1+roundtrip_cost_pct/100.0)
            stop=max(stop,breakeven_stop)
            if c.vwap>0 and c.vwap<c.price:
                stop=max(stop,c.vwap-0.20*max(c.atr,initial_risk))

        # +1.5R부터 R 기반 추세 트레일링.
        if r_mult>=self.trail_start_r:
            trail_r=1.00 if r_mult<2.5 else 0.75 if r_mult<4.0 else 0.55
            stop=max(stop,float(p["highest"])-trail_r*initial_risk)

        p["stop"]=stop

        # 장 후반 강한 DAY는 SWING으로 승격.
        if p.get("trade_type","DAY")=="DAY" and self._should_promote_to_swing(c,p):
            p["trade_type"]="SWING"
            self._log(
                "US_DAY_TO_SWING",symbol=c.symbol,price=round(c.price,4),
                score=round(c.score,2),rvol=round(c.rvol,3),
                day_high=round(c.day_high,4),vwap=round(c.vwap,4)
            )

        reasons=[]
        exit_score=0
        if c.vwap and c.price<c.vwap:
            exit_score+=2;reasons.append("VWAP_LOSS")
        if c.rvol>=1.5 and c.price<float(p["highest"])-0.65*max(c.atr,initial_risk):
        ...
        if c.price<=stop:
            p["weak_since"]=None
            self._queue_sell(c,p,"AUTO_EXIT:STOP"+(("/"+"/".join(reasons[:3])) if reasons else ""))
            return

        # 복합약화는 일정 시간 지속될 때만 청산.
        if exit_score>=5:
            if not p.get("weak_since"):
                p["weak_since"]=self.now_fn().isoformat()
                return
            try:
                weak_age=(self.now_fn()-datetime.fromisoformat(p["weak_since"])).total_seconds()
            except Exception:
                weak_age=0
            if weak_age>=self.weak_confirm_seconds:
                self._queue_sell(c,p,"AUTO_EXIT:CONFIRMED_WEAK/"+"/".join(reasons[:4]))
        elif exit_score<=2:
            p["weak_since"]=None

        # 승격되지 못한 DAY만 장 마감 5분 전에 정리.
        et=self._now_et()
        close=self._regular_close(et.date())
        close_dt=et.replace(hour=close.hour,minute=close.minute,second=0,microsecond=0)
        if (
            p.get("trade_type","DAY")=="DAY"
            and et>=close_dt-timedelta(minutes=5)
            and c.symbol not in self.state["pending_orders"]
        ):
            self._queue_sell(c,p,"DAY_EOD")
    ...
    def summary(self):
        with self.lock:
            ...
            return {
                ...
                "entry_diagnostics":dict(self.entry_diagnostics),
                "strategy_note":"US V1.4: Gap은 참고점수 · VWAP 눌림회복/PM고점/추세재돌파 진입 · +1R 전 구조손절 유지 · +1R 본전보호 · +1.5R 추세트레일 · 복합약화 지속확인 · 강한 DAY는 장막판 SWING 승격 · 재진입 쿨다운/일일 진입횟수 제한.",
                ...
                "load_guard":{"universe":len(self.universe),"candidate_limit":self.candidate_limit,
                              ...
                              "max_entries_per_symbol_day":self.max_entries_per_symbol_day,
